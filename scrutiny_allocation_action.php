...
{
	die("Invalid session Login again");
}
if($_SESSION['user'] !='' and $_SESSION['location'] !='')
{
...
$payment_accept='Y';
$sxx='NA';
$st=$dbonline->prepare("select count(*) from e_case_detail where scrutiny_comp1 is null and scrutiny_comp2 is null and scrutiny_comp3 is null and location_id=? and payment_accept=? and filing_no !=? and LENGTH(filing_no) = 16 and back_log='N'");
$st->bindParam(1, $location_access, PDO::PARAM_STR);
$st->bindParam(2, $payment_accept, PDO::PARAM_STR);
$st->bindParam(3, $sxx, PDO::PARAM_STR);
$st->execute();
 $count_online_filing = $st->fetchColumn();

if($count_online_filing =='' OR $count_online_filing =='0')
{
...
if($count_online_filing >0)
{
	//die('here');
	 $status='0';
	 $payment_accept='Y';
	 $sxx='NA';
	$st3=$dbonline->prepare("select * from e_case_detail where scrutiny_comp1 is null and scrutiny_comp2 is null and scrutiny_comp3 is null and location_id=? and payment_accept=? and filing_no !=? and LENGTH(filing_no) = 16 and back_log='N'");
	$st3->bindParam(1, $location_access, PDO::PARAM_STR);
	$st3->bindParam(2, $payment_accept, PDO::PARAM_STR);
	$st3->bindParam(3, $sxx, PDO::PARAM_STR);
	$st3->execute();

	$successfully_ins = array();
	$unsuccessfully_ins = array();
	while ($row = $st3->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
	{
			...
			//check case already exist in scrutiny and e_case_detail_local
			$st33=$db->prepare("select * from $schemas.scrutiny where filing_no=?");
		$st33->bindParam(1, $E_filing_no, PDO::PARAM_STR);
		$st33->execute();
		 $count = $st33->rowCount();
		/*while ($row666 = $st33->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
		{
		
			     $filling_no_check_scr=$row666['filing_no']; 
		}*/
		if($count==0)
		//if($filling_no_check_scr=='' )
		{
			...
			try{
               ...
               $st13=$db->prepare("insert into  $schemas.scrutiny (filing_no)values (?)");
  		$st13->bindParam(1, $E_filing_no, PDO::PARAM_STR);
  		$st13->execute();
		
		 $sttt=$dbonline->prepare("update e_case_detail set scrutiny_comp2='1' where filing_no=?");
		$sttt->bindParam(1, $E_filing_no, PDO::PARAM_STR);
		$sttt->execute();
...
echo $message = 'Successfully allocated:'.$E_filing_no;
$successfully_ins[]=$E_filing_no;
continue;
...
//die();
			...
			echo $e->getMessage();
				//throw $e;
			echo $message = 'Problem occured:'.$E_filing_no;
			$unsuccessfully_ins[]=$E_filing_no;
            continue;
		}   
...
//header("Location:./index.php");
//$successfully_ins = '666777';
//$msg=base64_encode($successfully_ins);
//$msg1=htmlspecialchars($message.'-'.$msg);
//$hash=base64_encode($msg1);
