...
if($type == 'save_remark'){
	...
	try{
		...
		if(!empty($get_case_detail)){
			...
			// apl-02 form start
			 ...
			 $save_file = file_put_contents($pp_path, $outputff);
			  ...
			  if(!$save_apl)
			  {
				...
				$response = array( 
				'status' => 0, 
				'message' => 'some error occurred in saving apl-02A' 
				);
				echo json_encode($response); die;
			  }
			...
			$apl_form= $db->prepare("update e_case_detail set apl_02b_form_path = ?, apl_02b_accept_reject = 1, pdf_hash = ?,accepted_rejected_at = now()  where filing_no = ?");
	...
	if($type == 'reject_case'){
		try{
			...
			if(!empty($get_case_detail)){
				...
				if($reject_type == '5' || $reject_type == '2'){
					...
					if($reject_type == '2'){
						$date_text = "Date of Dismissal:";
						$apl02_reject_type = 3;
						$apl_reject_text = "Case Dismissed as Withdrawn";
					}
					else{
						$date_text = "Date of Deferred:";
						$apl02_reject_type = 5;
						$apl_reject_text = "Case Deferred due to Wrong Applicability of Place of Supply";
					}

					$pdf_html = '<div style="text-align:center;"><p>Form GST APL-02A Part B</p><p><b>Final Acknowledgement for registration of Appeal/Application</b></p><p>Your Appeal/application filed vide provisional acknowledgment reference number '.$filing_no.' dated '.date('d/m/Y',strtotime($filed_date)).' has been '.$apl_reject_text.'</p></div><table style="width:100%"><tr><td><b>'.$date_text.'  '.date('d/m/Y').'</b></td><td style="text-align:right"><b>AR/JR/DR/R<br/>GSTAT: '.$bench_name .' Bench</b></td></tr></table>';
						 ...
						 $dompdf->loadHtml($pdf_html);
						  ...
						  $pdf_hash = hash_file('sha256', $pp_path);
						$apl_form= $db->prepare("update e_case_detail set apl_02b_form_path = ?, apl_02b_accept_reject = ?, pdf_hash = ?,accepted_rejected_at = now()  where filing_no = ?");
						$apl_form->bindParam(1, $pp_path, PDO::PARAM_STR);
						$apl_form->bindParam(2, $apl02_reject_type, PDO::PARAM_STR);
						$apl_form->bindParam(3, $pdf_hash, PDO::PARAM_STR);
						$apl_form->bindParam(4, $filing_no, PDO::PARAM_STR);
						$apl_form->execute();
