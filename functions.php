...
function generate_case_no($schemas,$db,$case_type,$case_year,$case_no,$location_code){
	$case_short_name = $db->prepare("select case_type_desc from case_type where id=?");
	...
	$case_no=$case_short_name.' No. '.$case_no."/".$case_year;
	return $case_no;
}
...
function get_connected_cases($schemas,$db,$filing_no){
	$stQ = $db->prepare("select * from $schemas.connected_cases where filing_no = ? and status = 'C' ");
	$stQ->bindParam(1, $filing_no, PDO::PARAM_STR);
	$stQ->execute();
	$connected_cases=$stQ->fetchAll();
...
function counsel_parties($schemas,$db,$dbonline,$case_type_array,$conn_case_type,$conn_filing_no,$party_flag){
	$text = '';
...
$st12=$dbonline->prepare("select * from e_more_representative where filing_no=? and party_flag=? and display='t'");
	...
	$text .= "<font face='Verdana' size ='2'>" ;
	  $text .= strtoupper($pet_advname22).'<br>';
	}	  
	$text .= '</font>';
return $text;	
}
...
function selected_child_cases($schemas,$db,$filing_no,$status,$listing_date,$bench_no,$list_flag,$court_no){
	$stq = $db->prepare("select vcc.filing_no,cd.case_no,cd.case_type,cd.case_year,ct.short_name as case_type_short_name,mlc.short_name from $schemas.viewable_child_cases as vcc 
						LEFT JOIN $schemas.case_detail as cd on cd.filing_no = vcc.filing_no 
						LEFT JOIN case_type as ct on ct.id = cd.case_type 
						LEFT JOIN mater_location_city as mlc ON mlc.city_id = cd.location_code
						where vcc.parent_filing_no = ? and vcc.display = ? and vcc.listing_date = ? and vcc.bench_no = ? and vcc.list_flag = ? and vcc.court_no=?");
	$stq->bindParam(1, $filing_no, PDO::PARAM_STR);
	$stq->bindParam(2, $status, PDO::PARAM_STR);
...
function insert_selected_child_cases($schemas,$db,$filing_no,$child_filing_no,$listing_date,$bench_no,$list_flag,$court_no,$bench_nature,$entry_date,$user_id,$display,$username){
	$stq = $db->prepare("insert into $schemas.viewable_child_cases (filing_no,parent_filing_no,listing_date,court_no,bench_no,bench_nature,entry_date,user_id,display,username,list_flag) values (?,?,?,?,?,?,?,?,?,?,?)");
...
function show_selected_child_cases($schemas,$db,$filing_no,$status,$listing_date,$bench_no,$list_flag,$court_no){
	$cases = selected_child_cases($schemas,$db,$filing_no,$status,$listing_date,$bench_no,$list_flag,$court_no);
...
function generate_case_no_by_filing_no($schemas,$db,$filing_no){
	$query = "SELECT
			 coalesce(ct.short_name,'')||' No '|| coalesce(cd.case_no,'')||'/'||coalesce(cd.case_year,'')
			FROM
...
function child_cases($schema,$db,$filing_no,$status){
	$select = $db->prepare("select cd.filing_no,cd.case_no,cd.case_type,cd.case_year,ct.short_name from $schema.case_detail as cd left join case_type as ct on ct.id = cd.case_type where cd.ia_ma_filing_no = ? and cd.status = ?");
	$select->bindParam(1, $filing_no, PDO::PARAM_STR);
...
function insert_final_child_cases($schemas,$db,$filing_no,$child_filing_no,$listing_date,$bench_no,$list_flag,$court_no,$bench_nature,$user_id,$username,$child_or_connected){
	$stq = $db->prepare("insert into $schemas.final_child_and_connected_cases (filing_no,parent_filing_no,listing_date,court_no,bench_no,bench_nature,user_id,child_or_connected,username,list_flag) values (?,?,?,?,?,?,?,?,?,?)");
	$stq->bindParam(1, $child_filing_no, PDO::PARAM_STR);
	$stq->bindParam(2, $filing_no, PDO::PARAM_STR);
	$stq->bindParam(3, $listing_date, PDO::PARAM_STR);
	$stq->bindParam(4, $court_no, PDO::PARAM_STR);
...
function show_selected_final_child_cases($schemas,$db,$filing_no,$display,$listing_date,$bench_no,$list_flag,$court_no,$child_or_connected){
	$cases = selected_final_cases($schemas,$db,$filing_no,$display,$listing_date,$bench_no,$list_flag,$court_no,$child_or_connected);
	foreach($cases as $key=>$value){
		$case_nos[] = generate_case_no_by_filing_no($schemas,$db,$value['filing_no']);
	}
	$case_no = '';
...
function selected_final_cases($schemas,$db,$filing_no,$display,$listing_date,$bench_no,$list_flag,$court_no,$child_or_connected){
	$stq = $db->prepare("select fcc.filing_no,cd.case_no,cd.case_type,cd.case_year,ct.short_name as case_type_short_name,mlc.short_name from $schemas.final_child_and_connected_cases as fcc 
						LEFT JOIN $schemas.case_detail as cd on cd.filing_no = fcc.filing_no 
						LEFT JOIN case_type as ct on ct.id = cd.case_type 
						LEFT JOIN mater_location_city as mlc ON mlc.city_id = cd.location_code
						where fcc.parent_filing_no = ? and fcc.display = ? and fcc.listing_date = ? and fcc.bench_no = ? and fcc.list_flag = ? and fcc.court_no=? and fcc.child_or_connected = ?");
	$stq->bindParam(1, $filing_no, PDO::PARAM_STR);
