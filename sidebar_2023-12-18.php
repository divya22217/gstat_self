...
?>
  ...
  <style>
    ...
    .topmenu .hassubmenu:hover .submenu,
    .topmenu .hassubsubmenu:hover .subsubmenu {
      display: block;
    }
    ...
    <ul class="nav navbar-nav topmenu">
      ...
      if ($localadmin == '2') { ?>
        <li class="hassubmenu"><a href="">Menu</a>
          <ul class="submenu">
            <li><a href="reg/create_menu_r.php">Create Menu</a></li>
            <li><a href="reg/create_submenu_r.php">Create Submenu</a></li>
            <li><a href="reg/menu_list.php">View Menu</a></li>
            <li><a href="reg/submenu_list.php">View Submenu</a></li>
          </ul>
        </li>
      <?php }
      if ($localadmin == '1') { ?>
        ...
        <?php }
      $topmenu_sql = $db->prepare("select distinct P.menu_id,P.menu_name, P.parent_menu, P.priority  from public.menu_r P, public.link_r L where P.menu_id=L.menu_id and L.user_id='$userid' and P.display='TRUE' order by P.priority");
      //print_r($topmenu_sql);
      $topmenu_sql->execute();
      while ($topmenu_sql_result = $topmenu_sql->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT)) {
        ...
        ?>
          <li class="hassubmenu"><a href=""><?php echo htmlspecialchars($topmenu_sql_result['menu_name']) ?></a>
            <ul class="submenu">
              <?php
              $submenu_sql = $db->prepare("select * from public.submenu_r S, public.link_r L where S.menu_id = '$menu_id' and S.submenu_id=L.submenu_id and L.user_id='$userid' and S.display='TRUE' order by S.priority");
              //print_r($submenu_sql);
              $submenu_sql->execute();
              while ($submenu_sql_result = $submenu_sql->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT)) {
              ?>
                <li><a href="<?php $_SERVER['DOCUMENT_ROOT'] . 'nclat_audit'; ?>/nclat_audit/<?php echo htmlspecialchars($submenu_sql_result['submenu_link']); ?>"><?php echo htmlspecialchars($submenu_sql_result['submenu_name']); ?></a></li>
              <?php } ?>
            </ul>
          </li>
      <?php }
      ...
      <li class="dropdown user user-menu">
        <a href="#" class="dropdown-toggle" data-toggle="dropdown">
          ...
          <span class="hidden-xs">
            <?php
            echo htmlspecialchars(strtoupper($_SESSION['user'])) . '(' . htmlspecialchars(strtoupper($_SESSION['user_actual_name'])) . ')';
            ?>
          </span>
        </a>
        <ul class="dropdown-menu">
