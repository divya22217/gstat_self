...
if($disposal_nature != '46'){
	...
	if($condonation_delay_flag  && $disposal_nature == '3'){
		...
		if(($disposal_nature == '38' || $disposal_nature == '52') && $is_main_case == 'M'){
			...
			$pdf_html = '<div style="text-align:center;"><p>Form GST APL-02A Part B</p><p><b>Final Acknowledgement for registration of Appeal/Application</b></p><p>Your Appeal/application filed vide provisional acknowledgment reference number '.$filing_no.' dated '.date('d/m/Y',strtotime($filed_date)).' has been '.$apl_text.'</p></div><table style="width:100%"><tr><td><b>Date of rejection:  '.date('d/m/Y',strtotime($disposal_date_new)).'</b></td><td style="text-align:right"><b>AR/JR/DR/R<br/>GSTAT: '.$bench_name .' Bench</b></td></tr></table>';
				...
				$apl_form= $db->prepare("update e_case_detail set apl_02b_form_path = ?, apl_02b_accept_reject = 2, pdf_hash = ?,accepted_rejected_at = now()  where filing_no = ?");
		...
		if($is_main_case == 'M'){
			...
			if($disposal_nature == '38' || $disposal_nature == '52'){
				$apl_02_A = " Please find attached APL-02A Part B";
