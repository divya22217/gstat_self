...
function search_validation(){
	with(document.frm)
	{
		...
		action = "case_type_filing_report.php";
		submit();
	}
}
	</script>
...
<tr>
	<th valign="top" colspan="16" align="center">
					<b><font face="Verdana" size="3"><u>Search by Case No. Wise Report</u></font> </b>
	</th>
	</tr>
...
<input type="text"  placeholder="yyyy" maxlength="4" size="4" name="case_year" value="<?php print htmlentities(htmlspecialchars($case_year)); ?>" onFocus="SetBg(this)" onBlur="UnSetBg(this)">
<input type="submit" id="submit11" name="submit11" onclick="search_validation();">
</td>
<tr>
<th>S No.</th>
<th>Diary No.</th>
...
Party Detail
 ...
 <?php
 if(isset($_POST['submit11']) && $_POST[case_type] !='' && $_POST[case_no] !=''  && $_POST[case_year] !='' && $_POST[bench_name] !=''){
	 //print_r($_POST);die('g');
  ...
  $case_year = htmlentities(htmlspecialchars($_REQUEST['case_year']));
   ...
   $bench_id = htmlentities(htmlspecialchars($_POST['bench_name']));
//print_r($bench_id);

 $casedetailsql = $db->prepare("select * from $schemas.case_detail where location_code = ? and case_type = ? and case_year= ? and case_no = ?");
 //$casedetailsql = $db->prepare("select * from $schemas.case_detail where location_code = '$bench_id' and case_type= '$case_type' and case_year= '$case_year' and case_no = '$case_no'");
  //print_r($casedetailsql);die('p');
 $casedetailsql->bindParam(1, $bench_id, PDO::PARAM_INT);
 $casedetailsql->bindParam(2, $case_type, PDO::PARAM_INT);
 $casedetailsql->bindParam(3, $case_year, PDO::PARAM_INT);
 $casedetailsql->bindParam(4, $case_no, PDO::PARAM_INT);
