...
$penordis_main = stripslashes($_POST['penordis_conn']);

$restoration_action_type = stripslashes($_POST['restoration_action_type']);
//echo "kkkkkk".$restoration_action_type;die;
$case_type_rst = stripslashes($_POST['case_type_rst']);
//echo $case_type_rst;die;

$action_type_main = stripslashes($_POST['action_type_conn']);

$purpose_code_main = stripslashes($_POST['purpose_code_conn']);

$date_main = stripslashes($_POST['date_conn']);    //conn case next list date
...
$remarks_main = stripslashes($_POST['remarks_conn']);

$remarks_cir_liq_main = stripslashes($_POST['remarks_cir_liq_conn']);

$cir_liq_main = stripslashes($_POST['cir_liq_conn']);

$filing_no_main = stripslashes($_POST['filing_no_conn']);

$bench_no_conn = stripslashes($_POST['bench_no_conn']);
...
$disposal_nature_main = stripslashes($_POST['disposal_nature_conn']);

$date_dis_main = stripslashes($_POST['date_dis_conn']);
...
$st13->bindParam(6, $bench_no_conn, PDO::PARAM_STR);
...
$st13->execute();
 ...
 $update_case_allocation_result->bindParam(6, $bench_no_conn, PDO::PARAM_STR);
...
purpose=?,deal_cd=?,entry_date=?,next_list_date=?,listed=?,remarkscirp=?,remark_type=? where filing_no=? and listing_date=?";
...
$st->bindParam(6, $remarks_cir_liq_main, PDO::PARAM_STR);
$st->bindParam(7, $cir_liq_main, PDO::PARAM_STR);
$st->bindParam(8, $filing_no_main, PDO::PARAM_STR);
$st->bindParam(9, $listing_date_main, PDO::PARAM_STR);
$st->execute();
echo "Case Proceeding successfully done!";
}

/*  restoration  */
if(($case_type == 6 || $case_type == 22) && ($penordis_main='D' || $penordis_main='d') && $disposal_nature_main == '42'){
    $update_case_proc_rst_sql = "update $schemas.case_proceeding set restoration_action_type=?  where filing_no=? and listing_date=?";
    $update_case_proc_rst_result=$db->prepare($update_case_proc_rst_sql);
    $update_case_proc_rst_result->bindParam(1, $restoration_action_type, PDO::PARAM_STR);
    $update_case_proc_rst_result->bindParam(2, $filing_no_main, PDO::PARAM_STR);
    $update_case_proc_rst_result->bindParam(3, $listing_date_main, PDO::PARAM_STR);
    $update_case_proc_rst_result->execute();

    $get_main_case= $dbonline->prepare("select in_filingno from e_case_detail where filing_no=? and (case_type='6' or case_type='22') ");
    $get_main_case->bindParam(1, $filing_no_main, PDO::PARAM_STR);
    $get_main_case->execute();
    $main_fil_no = $get_main_case->fetchColumn();
    
    if($main_fil_no){
        $ins_rest_table="insert into $schemas.restoration_table
        (filing_no_rst,filing_no_main,disposal_dt_rst,entry_dt,user_id)
        VALUES (?,?,?,?,?)";
        
        $ins_rest_table=$db->prepare($ins_rest_table);
    //$st13 = array($filing_no,$list_date_link,$purpose_old,$court_no,$bench_nature,$bench_no,$purpose_code_next,$criteria,$next_list_date,$action_type,$pen_dis,$curdate,$remarks,$sessionUserType);
    $ins_rest_table->bindParam(1, $filing_no_main, PDO::PARAM_STR);
    $ins_rest_table->bindParam(2, $main_fil_no, PDO::PARAM_STR);
    $ins_rest_table->bindParam(3, $date_dis_main, PDO::PARAM_STR);
    $ins_rest_table->bindParam(4, $server_date, PDO::PARAM_STR);
    $ins_rest_table->bindParam(5, $sessionUserType, PDO::PARAM_STR);
    $ins_rest_table->execute();
    
    $status_rst = 'P';
    $update_case_det_status = "update $schemas.case_detail set status=?  where filing_no=? and status='D'";
    $update_case_det_status=$db->prepare($update_case_det_status);
    $update_case_det_status->bindParam(1, $status_rst, PDO::PARAM_STR);
    $update_case_det_status->bindParam(2, $main_fil_no, PDO::PARAM_STR);
    $update_case_det_status->execute();
    }
}

$get_rst_case= $dbonline->prepare("select filing_no from e_case_detail where in_filingno=? and (case_type='6' or case_type='22') ");
$get_rst_case->bindParam(1, $filing_no_main, PDO::PARAM_STR);
$get_rst_case->execute();
$rst_fil_no = $get_rst_case->fetchColumn();
...
$dbonline->commit();
    ...
    echo $e->getMessage();
        ...
        echo $message = 'There is some problem while doing proceeding...';
        die();
