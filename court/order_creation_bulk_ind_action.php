...
//echo "</pre>"; print_r($_POST);
    ...
    if ($order_to_update == 'update') {
        ...
        $napa_center_penalty = $napa_state_penalty = $napa_intg_penalty = $napa_cess_penalty = $napa_total_penalty = null;
        if(!empty($napa_order) && $napa_order == '4'){
            $napa_center_penalty = (!empty($_POST['napa_center_penalty']))?htmlspecialchars($_POST['napa_center_penalty']):0;
            $napa_state_penalty = (!empty($_POST['napa_state_penalty']))?htmlspecialchars($_POST['napa_state_penalty']):0;
            $napa_intg_penalty = (!empty($_POST['napa_intg_penalty']))?htmlspecialchars($_POST['napa_intg_penalty']):0;
            $napa_cess_penalty = (!empty($_POST['napa_cess_penalty']))?htmlspecialchars($_POST['napa_cess_penalty']):0;
            $napa_total_penalty = $napa_center_penalty+$napa_state_penalty+$napa_intg_penalty+$napa_cess_penalty;

            $tax_states = (!empty($_POST['tax_state_id']))?$_POST['tax_state_id']:[];
            $tax_amounts = (!empty($_POST['tax_amount']))?$_POST['tax_amount']:[];
...
values(?,?,?,?,?,?,?,?)");
        ...
        $sql2 = "select order_ref_no,order_date from $schemas.order_daily where filing_no = ? and item_no = ? ";
        ...
        $update_order = $db->prepare("update $schemas.order_daily set order_tribunal= ?, personal_hearing=?, brief_order = ?, order_status = ?, det_amount_tax = ?, det_amount_interest = ?, det_amount_penalty = ?, det_amount_fees = ?, det_amount_others = ?, det_amount_refund = ?, direction_subject=? , authority_name = ?, demand_quantified = ?, remand_order = ?, napa_order = ?, order_in_brief_napa = ? where item_no = ? and filing_no = ?");
            ...
            $query = "insert into $schemas.napa_panelty_imposed_his (main_id,filing_no,order_date,item_no,central_tax,state_tax,integrated_tax,cess,total_tax,main_created_at,main_updated_at)  (select id,filing_no,order_date,item_no,central_tax,state_tax,integrated_tax,cess,total_tax,created_at,updated_at from $schemas.napa_panelty_imposed where filing_no = ? and order_date = ? and item_no = ?)";
            $st = $db->prepare($query);
            $st->bindParam(1, $filing_no, PDO::PARAM_STR);
            $st->bindParam(2, $order_date, PDO::PARAM_STR);
            $st->bindParam(3, $order_ref_no, PDO::PARAM_STR);
            $st->execute();

            $query = "insert into $schemas.napa_igst_penalty_his (main_id,filing_no,order_date,item_no,state_id,tax_amount,main_created_at,main_updated_at)  (select id,filing_no,order_date,item_no,state_id,tax_amount,created_at,updated_at from $schemas.napa_igst_penalty where filing_no = ? and order_date = ? and item_no = ?)";
            $st = $db->prepare($query);
            $st->bindParam(1, $filing_no, PDO::PARAM_STR);
            $st->bindParam(2, $order_date, PDO::PARAM_STR);
            $st->bindParam(3, $order_ref_no, PDO::PARAM_STR);
            ...
            $query = "delete from $schemas.napa_igst_penalty where filing_no = ? and order_date = ? and item_no = ?";
            $st = $db->prepare($query);
            $st->bindParam(1, $filing_no, PDO::PARAM_STR);
            $st->bindParam(2, $order_date, PDO::PARAM_STR);
            $st->bindParam(3, $order_ref_no, PDO::PARAM_STR);
            ...
            $query = "delete from $schemas.napa_panelty_imposed where filing_no = ? and order_date = ? and item_no = ?";
            $st = $db->prepare($query);
            $st->bindParam(1, $filing_no, PDO::PARAM_STR);
            $st->bindParam(2, $order_date, PDO::PARAM_STR);
            $st->bindParam(3, $order_ref_no, PDO::PARAM_STR);
            ...
            $query = "insert into $schemas.napa_panelty_imposed (filing_no,order_date,item_no,central_tax,state_tax,integrated_tax,cess,total_tax) values (?,?,?,?,?,?,?,?)";
            $st = $db->prepare($query);
            $st->bindParam(1, $filing_no, PDO::PARAM_STR);
            $st->bindParam(2, $order_date, PDO::PARAM_STR);
            $st->bindParam(3, $order_ref_no, PDO::PARAM_STR);
            $st->bindParam(4, $napa_center_penalty, PDO::PARAM_STR);
            $st->bindParam(5, $napa_state_penalty, PDO::PARAM_STR);
            $st->bindParam(6, $napa_intg_penalty, PDO::PARAM_STR);
            $st->bindParam(7, $napa_cess_penalty, PDO::PARAM_STR);
            $st->bindParam(8, $napa_total_penalty, PDO::PARAM_STR);
            $st->execute();


            foreach($tax_states as $key=>$value){
                $state_id = (!empty($value))?$value:0;
                $tax_amount = (!empty($tax_amounts[$key]))?$tax_amounts[$key]:0;  
                $query = "insert into $schemas.napa_igst_penalty (filing_no,order_date,item_no,state_id,tax_amount) values (?,?,?,?,?)";
                $st = $db->prepare($query);
                $st->bindParam(1, $filing_no, PDO::PARAM_STR);
                $st->bindParam(2, $order_date, PDO::PARAM_STR);
                $st->bindParam(3, $order_ref_no, PDO::PARAM_STR);
                $st->bindParam(4, $state_id, PDO::PARAM_STR);
                $st->bindParam(5, $tax_amount, PDO::PARAM_STR);
                $st->execute();
            }
