...
<tr>
		...
		<span class="error"> <b> <?php  echo "RECORD ALREADY EXISTS IN OL"; ?></span></font>
		</td>
		</tr>
		<tr>
		<td height="30" align="center" cellpadding="0" colspan="6">
...
<tr>
 <td align="center">
<font face="Verdana" color="red">*</span> </font><font face="Verdana" size="2">OL NAME/DESC:
</td>
<td>
<select name="role">-->
<option value="">Select OL/DESC</option>
<?php
$st = $dbonline->prepare("select * from e_master_ol where display = 'TRUE' order by ol_id asc");
$st->execute();
while ($row = $st->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
{
$ctc=htmlspecialchars($row['ol_id']);
if($role == $ctc)
{
print "<option value=".htmlspecialchars($row['ol_id'])." selected>".htmlspecialchars($row['ol_id'])."--".htmlspecialchars($row['ol_name'])."---".htmlspecialchars($row['ol_desc'])."</option>";
}
else
{
print "<option value=".htmlspecialchars($row['ol_id']).">".htmlspecialchars($row['ol_id'])."--".htmlspecialchars($row['ol_name'])."---".htmlspecialchars($row['ol_desc'])."</option>";
}
}
?>
</select>
