...
<input type="hidden" name="old_ol_id" value="<?php print htmlentities(htmlspecialchars($role2)); ?>">
<?php
...
<?php
//echo"oohh";
$st = $dbonline->prepare("select * from public.e_master_ol where display = 'T' order by ol_id asc");
print_r($st);
$st->execute();
while ($row = $st->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
{
