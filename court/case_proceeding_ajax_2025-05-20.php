...
if($_SESSION['user'] !='' and $_SESSION['location'] !='')
{
	...
	<table>
		...
		if($list_with_defect == 1 && empty($registration_date)){
			...
			$query = "select * from $schemas.master_action where status='D' and action_code in (2,3,23,38,39,41,51)";
			}else{
				$query = "select * from $schemas.master_action where status='D' and action_code in (2,3,23,38,39,41)";
			}
		}
		else{
			$query = "select * from $schemas.master_action where status='D' and action_code not in (38)";
		}
