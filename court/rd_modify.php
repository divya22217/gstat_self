...
<input type="hidden" name="old_rd_id" value="<?php print htmlentities(htmlspecialchars($role2)); ?>">
<?php
...
<tr>
 ...
 <tr>
 <td align="center">
<font face="Verdana" color="red">*</span> </font><font face="Verdana" size="2">RD REGION:
</td>
<td>
<select name="role">
<?php
//echo"oohh";
$st = $dbonline->prepare("select * from public.e_master_rd where display = 'T' order by rd_id asc");
print_r($st);
$st->execute();
while ($row = $st->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
{
$ctc=htmlspecialchars($row['rd_id']);
if($role2 == $ctc)
{
print "<option value=".htmlspecialchars($row['rd_id'])." selected>".htmlspecialchars($row['rd_id'])."--".htmlspecialchars($row['rd_region'])."</option>";
}
else
{
print "<option value=".htmlspecialchars($row['rd_id']).">".htmlspecialchars($row['rd_id'])."--".htmlspecialchars($row['rd_region'])."</option>";
}
}
?>
</select>
