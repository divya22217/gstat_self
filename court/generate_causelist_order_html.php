...
function popsurety_pending_report(cfy)

    {
    		...
    		var url = "./order_creation_bulk_ind1.php?no="+cfy;
    		 ...
    		 childWindow = window.open(url,"_blank","directories=no, status=no,width=800, height=800, left=100, scrollbars=yes");   
...
<a href="./order_creation_bulk_ind.php">
...
/*if($snoc > 99 && $snoc < 200 && $counter < 1){
?>
<tr><td colspan="7" align="left"><b>ORDINARY LIST:</b></td></tr>
<?php
$counter ++;
}*/
...
$filing_no_link=$filing_no."@".$courtnoc."@".$list_date."@".$bench_naturec."@".$list_flag."@".$purpose."@".$bench_noc."@".$bench_naturec."@".$snoc;

if(empty($in_case_nor) && $ia_main_filingno == ''){
	$ordDet = $causelist->getOrderDetails($filing_no, $schemas, $db, $list_date_db, $bench_noc);
}
 ...
 for($i=0;$i<$len_array;$i++) {
   if($in_case_nor[$i]['case_no']!=''){
	 $ordDet = $causelist->getOrderDetails($in_case_nor[$i]['fil_no'], $schemas, $db, $list_date_db, $bench_noc);  
   }
 }
  }
  
  if($ia_main_filingno!=''){ 
  $ordDet = $causelist->getOrderDetails($ia_main_filingno, $schemas, $db, $list_date_db, $bench_noc);
  }
...
?>
  <tr>
    ...
    <td><a  href="javascript:popsurety_pending_report('<?php echo htmlspecialchars($filing_no_link) ; ?>');" style="cursor: pointer">

<?php if($ordDet==1){?><font color="green" face="verdana" size="2"><?php }else{ ?><font color="#900C3F" face="verdana" size="2"><?php } echo '<b>'.$main_case_no.'</b>' ?></font></a>

<?php if($in_filingno!=''){

      $len_array = count($in_case_no);
	for($i=0;$i<$len_array;$i++) {
		if($in_case_no[$i]['case_no']!=''){

      $in_filing_no_link=$in_case_no[$i]['fil_no']."@".$courtnoc."@".$list_date."@".$bench_naturec."@".$list_flag."@".$purpose."@".$bench_noc;
      ...
      ?>
      <br>
      <a href="javascript:popsurety_pending_report('<?php echo htmlspecialchars($filing_no_link) ; ?>');" style="cursor: pointer">
      
       <?php if($ordDet==1){?><font color="green" face="verdana" size="2"><?php }else{ ?><font color="#900C3F" face="verdana" size="2"><?php } echo '<b>'.$in_case_no[$i]['case_no'].'</b>'; ?></font></a>
...
$in_filingno = '';
 ...
 for($i=0;$i<$len_array;$i++) {
   if($in_case_nor[$i]['case_no']!=''){
	   ...
	   //----------order------------------//
	   $in_filing_no_link=$in_case_nor[$i]['fil_no']."@".$courtnoc."@".$list_date."@".$bench_naturec."@".$list_flag."@".$purpose."@".$bench_noc;
	   //---------order-------------------//
...
<?php if($ordDet==1){?><font color="green" face="verdana" size="2"><?php }else{ ?><font color="#900C3F" face="verdana" size="2"><?php } echo '<b>'.$in_case_nor[$i]['case_no'].'</b>';?>
</font></a>
<?php
		}	
 }
 ...
 for($i=0;$i<$len_array;$i++) {
   if($in_case_nocp[$i]['case_no']!=''){
	   ...
	   //----------order------------------//
	   $in_filing_no_link=$in_case_nocp[$i]['fil_no']."@".$courtnoc."@".$list_date."@".$bench_naturec."@".$list_flag."@".$purpose."@".$bench_noc;
	   //---------order-------------------//
...
<?php if($ordDet==1){?><font color="green" face="verdana" size="2"><?php }else{ ?><font color="#900C3F" face="verdana" size="2"><?php } echo '<b>'.$in_case_nocp[$i]['case_no'].'</b>';?>
</font></a>
<?php
		}	
 }
 ...
 //-----------------------------------------------------------------------------------//
  ...
  if($ia_main_filingno!=''){ 
	 ...
	 ?>
<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font size="2"><b>In</b></font>
<br><a href="javascript:popsurety_pending_report('<?php echo htmlspecialchars($ia_filing_no_link);?>');" style="text-decoration: none;" > <?php if($ordDet==1){?><font color="green" face="verdana" size="2"><?php }else{ ?><font color="#900C3F" face="verdana" size="2"><?php } echo '<b>'.$ia_main_case_no.'</b>' ?></font></a><br><br>
<?php
$ia_main_filingno = '';	
 }
?>
...
<?php if($ia_filingno!=''){ 
	 $ia_filing_no_link=$ia_filingno."@".$courtnoc."@".$list_date."@".$bench_naturec."@".$list_flag."@".$purpose."@".$bench_noc;
	 ?>
<br><br>
<?php
foreach($ia_filingno_case_no as $key => $value)
{
	?>
  <a href="javascript:popsurety_pending_report('<?php echo htmlspecialchars($filing_no_link);?>');" style="text-decoration: none;" > <?php if($ordDet==1){?><font color="green" face="verdana" size="2"><?php }else{ ?><font color="#900C3F" face="verdana" size="2"><?php } echo '<b>';print_r($value);echo '</b>'; ?></font></a><br><br>
	<?php
}
 }
 ...
 for($i=0;$i<$len_array;$i++) {
   if($connected_cases[$i]['case_no']!=''){
	   ...
	   //----------order------------------//
	   $in_filing_no_link=$connected_cases[$i]['fil_no']."@".$courtnoc."@".$list_date."@".$bench_naturec."@".$list_flag."@".$purpose."@".$bench_noc;
	   //---------order-------------------//
...
?>
<!-------------order---------------->
<a href="javascript:popsurety_pending_report('<?php echo htmlspecialchars($in_filing_no_link) ; ?>');" style="cursor: pointer">
<!-------------order---------------->

<?php if($ordDet==1){?><font color="green" face="verdana" size="2"><?php }else{ ?><font color="#900C3F" face="verdana" size="2"><?php } echo '<b>'.$connected_cases[$i]['case_no'].'</b>';?>
</font></a>
<?php
		}	
 }
