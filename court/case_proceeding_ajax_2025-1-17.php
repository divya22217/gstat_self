	...
	<table>
		...
		<?php
			if($list_with_defect == 1 && empty($registration_date) && $list_before_link == '3')
				...
				$query = "select * from $schemas.master_action where status='P' and action_code not in (44,45)";
			else
		...
		<tr id="list_date_element<?php echo $filing_no; ?>">
		...
		if($list_with_defect == 1 && empty($registration_date))
			$query = "select * from $schemas.master_action where status='D' and action_code in (2,3,23,38,39,41)";
		else
			$query = "select * from $schemas.master_action where status='D'";
