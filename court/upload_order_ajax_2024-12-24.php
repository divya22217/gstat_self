						...
						if ($type == 'search_case_to_recuse_judge') {
							...
							if (!empty($case_detail) && count($case_detail) == 1) {
								...
								if(!empty($listing_data)){
									...
									$mj = $db->prepare("select * from $schema.master_judge where judge_code in ($judge_ids)order by judge_desg_code asc");
									$mj->execute();
									$master_judges = $mj->fetchAll();
								}else{
									$mj = $db->prepare("select * from $schema.master_judge order by judge_desg_code asc");
									$mj->execute();
									$master_judges = $mj->fetchAll();
								}
