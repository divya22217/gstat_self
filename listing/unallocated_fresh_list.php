...
function submitForm()
{
 	with(document.frm)
	{
		...
		action = "unallocated_fresh_list.php";
		submit();
	}
}
...
function submitForm21()
{
 	with(document.frm)
	{
		action = "unallocated_fresh_list_action.php";
		submit();
	}
}
...
<form name="frm" method="post" action="unallocated_fresh_list_action.php" >
...
?>
  ...
  <table cellspacing="0" align="center" cellpadding="2" border="1" width="95%" class="std">
	...
	//$sql1=$db->prepare("select main_case_ia_no,ia_flag,filing_no,case_type,pet_name,res_name,dt_of_filing,case_no,case_year,location_code from $schemas.case_detail where legal_aid is null and case_no is NULL and case_year='' and dt_of_filing < '2019-08-01' order by filing_no asc");
		$sql1=$db->prepare("select main_case_ia_no,ia_flag,filing_no,case_type,pet_name,res_name,dt_of_filing,case_no,case_year,location_code from $schemas.case_detail where legal_aid is null and (case_no is NULL OR case_no='')  and case_year='' order by filing_no asc");

	
	}else{
		
	///$sql1=$db->prepare("select main_case_ia_no,ia_flag,filing_no,case_type,pet_name,res_name,dt_of_filing,case_no,case_year,location_code from $schemas.case_detail where legal_aid is null and case_no is NULL and case_year='' and case_type='$search_case_type' and dt_of_filing < '2019-08-01' order by filing_no asc");
		$sql1=$db->prepare("select main_case_ia_no,ia_flag,filing_no,case_type,pet_name,res_name,dt_of_filing,case_no,case_year,location_code from $schemas.case_detail where legal_aid is null and  (case_no is NULL OR case_no='') and case_year='' and case_type='$search_case_type'   order by filing_no asc");
...
while ($row1 = $sql1->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
{
  ...
  if($ia_flag == '1' && $main_case_no_ia!=''){
	    $sql="select max(listing_date) as listing_date,court_no,bench_nature,bench_no from $schemas.case_allocation_temp where filing_no='$main_case_no_ia'";
...
<td><?php  echo htmlspecialchars($case_type_short_name); ?></td>
<!--<td><?php // echo htmlspecialchars($case_no_main); ?></td>-->
</tr>
