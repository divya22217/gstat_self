...
function submitForm()
{
 	with(document.frm)
	{
		...
		action = "court_unalloc_fresh.php";
		submit();
	}
}
...
function submitForm21()
{
 	with(document.frm)
	{
		action = "court_unalloc_fresh_action.php";
		submit();
	}
}
...
function submitForm2()
{
   ...
   <meta charset="utf-8">
    ...
    <title>Court Unallocated Fresh Case Listing</title>
...
<form name="frm" method="post" action="court_unalloc_fresh_action.php" >
...
<tr>
	 ...
	 $sql2=" select * from users where id='$sessionUserType' ";
...
?>
  ...
  <table cellspacing="0" align="center" cellpadding="2" border="1" width="95%" class="std">
	...
	//$sql1=$db->prepare("select * from $schemas.case_detail where    dt_of_filing >= '2019-09-01' and court_no='0' order by filing_no asc");
		$sql1=$db->prepare("select * from $schemas.case_detail where    court_no='0' and pet_occupation!='R'  order by filing_no asc");

	
	
	}else{
	//$sql1=$db->prepare("select * from $schemas.case_detail where    dt_of_filing >= '2019-09-01' and case_type='$search_case_type'  and court_no='0' order by filing_no asc");
		$sql1=$db->prepare("select * from $schemas.case_detail where   case_type='$search_case_type'  and court_no='0' and pet_occupation!='R' order by filing_no asc");
