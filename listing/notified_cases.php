    ...
    <!-- Content Header (Page header) -->
				...
				?>
					<section class="content-header">
						<center><b>Notified Cases</b></center>
					</section>
						...
						<div class="table-responsive" id="table_data">
							<table cellspacing="0" align="center" class="table no-margin table-bordered table-striped table-hover casealloc_table"   cellpadding="2" border="1" width="95%" class="std">
								<thead style="background-color:#00a65a;color:#ffffff;">
									...
									<th align="left" width="250"><B>Previous Listing Date</b></th>
									<th align="left" width="250"><B>Notified Date</b></th>
								...
								from $schemas.notified_cases as cp left join $schemas.case_detail as a  on a.filing_no = cp.filing_no
								where cp.notified_status = ? and cp.removed_by = ? order by cp.entry_date asc");
								$sql1->bindParam(1, $notified_status, PDO::PARAM_INT);
								$sql1->bindParam(2, $removed_by, PDO::PARAM_INT);
								$sql1->execute();
								...
								<tr>
									...
									<td>
										<?php
										if($notfied_date!='')
										{ echo date('d/m/Y',strtotime($notfied_date)); }?>
									</td>
									
									<td id="list_date_<?php echo $filing_no; ?>">
									<?php 
									$st="select max(next_list_date) from $schemas.case_proceeding where filing_no = ?";
									  $st=$db->prepare($st);
										$st->bindParam(1, $filing_no, PDO::PARAM_STR);
										$st->execute();
										$max_next_list = $st->fetchColumn();
										 if(!empty($max_next_list) && $max_next_list != $listing_date){
											echo "";
										 }else{  ?>
									...
									<br/>Next List Purpose : <select class='form_control' name="purpose_code_<?php echo $filing_no; ?>" id='purpose_code_<?php echo $filing_no; ?>'>
										...
										<input type="button" class="btn btn-sm btn-success" value="Save" onClick="save_listing_date('<?php echo $filing_no; ?>','<?php echo $listing_date; ?>')">
										 <?php } ?>
									</td>
...
<div id="comp_note" class="modal fade" role="dialog">
 ...
 <script src="../js/custom.js"></script>
	<script>
		...
		function save_listing_date(filing_no,listing_date){
		var next_listing_date = $("#next_list_date_"+filing_no).val();
		var next_list_purpose = $("#purpose_code_"+filing_no).val();
		$.ajax({
			type: "POST",
			url: "notified_ajax.php",
			data: {type:'save_next_list_date',filing_no:filing_no,listing_date:listing_date,next_listing_date:next_listing_date,next_list_purpose:next_list_purpose},
