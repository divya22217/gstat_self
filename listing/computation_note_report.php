    ...
    <!-- Content Header (Page header) -->
				...
				?>
					<section class="content-header">
						<center><b>Computation Note Report</b></center>
					</section>
						...
						<div class="table-responsive" id="table_data">
							<table cellspacing="0" align="center" class="table no-margin table-bordered table-striped table-hover casealloc_table"   cellpadding="2" border="1" width="95%" class="std">
								<thead style="background-color:#00a65a;color:#ffffff;">
									...
									<th align="left" width="700"><B>Status </b></th>
									<th align="left" width="250"><B>Date Of Registration</b></th>
									<th align="left" width="250"><B>Court No Allocated</b></th>
									<th align="left" width="250"><B>Main Case Court No Allocated</b></th>
									<th align="left" width="250"><B>Computation Note Detail</b></th>
								</thead>
								...
								$sql1=$db->prepare("select cn.*,a.case_no,a.case_type,a.case_year,a.pet_name,a.res_name,a.dt_of_filing,a.location_code,a.status,a.regis_date,a.main_case_ia_no, 
					s.first_listing_date,s.first_court_no from $schemas.computational_note as cn left join  $schemas.case_detail as a  on a.filing_no = cn.filing_no left join $schemas.scrutiny as s on s.filing_no = a.filing_no
					where s.is_computation_note_approved = '1' order by cn.entry_date desc");
								$sql1->execute();
								...
								<tr>
									...
									<td>
										<?php
											echo ($status == 'D')?'Disposed':'Pending';
										?>
									</td>
									<td>
										<?php
										$court_no_of_case = '';
										if(!empty($filing_no)){
											$get_from_proceeding = main_case_court_no_from_allocation($db,$schemas,$filing_no);
											if(!empty($get_from_proceeding)){
												$court_no_of_case = $get_from_proceeding;
											}
											if(!empty($court_no_of_case)){
												echo get_display_court_text($db,$schemas,$court_no_of_case);
											}
										}
										?>
									</td>
									...
									<td style='color:red;'>
										<?php if(!empty($for_listing))
										{ echo "For Listing : ".display_date_format($for_listing); }
										 if(!empty($for_court))
										{ echo "<br/>".get_display_court_text($db,$schemas,$for_court); } ?>
