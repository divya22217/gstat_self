...
if($_SESSION['user'] !='' && $_SESSION['location'] !='' && (isset($_POST['listing_date'])))
{
	...
	// }
	$sql2=" select * from $schemas.bench where  from_list_date =? and court_no = ? order by court_no asc";
	$bench_d=$db->prepare($sql2);
    $bench_d->bindParam(1, $court_date_new, PDO::PARAM_STR);
    if($_SESSION['menuaccess_codeall'] == 11 || $_SESSION['menuaccess_codeall'] == 6)
		$bench_d->bindParam(2, $court_no, PDO::PARAM_STR);
	else
		$bench_d->bindParam(2, $user_court, PDO::PARAM_STR);
		...
		$case_limit_avail =$limit_case-$counter;	
			...
			<?php 
				...
				$display_court_value = $display_court_value->fetchColumn();
				 echo $display_court_value.' Daily';
