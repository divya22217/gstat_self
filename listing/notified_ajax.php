...
if($sessionUserType){
	try{
		...
		$st1="insert into $schemas.case_proceeding_his (select * from $schemas.case_proceeding where next_list_date = ? and filing_no = ?)";
		$st1=$db->prepare($st1);
		...
		$update_proceeding="update $schemas.case_proceeding set next_list_date=? , next_list_purpose =? where next_list_date=? and filing_no = ?";
		...
		$update_notified="update $schemas.notified_cases set new_listing_date=? , removed_from_notified =now(),removed_by = ?,
							update_by_user_id = ?,update_by_user_name = ? where old_listing_date=? and filing_no = ?";
		$update_notified=$db->prepare($update_notified);
		$update_notified->bindParam(1, $next_listing_date, PDO::PARAM_STR);
		$update_notified->bindParam(2, $sessionUserType, PDO::PARAM_STR);
		$update_notified->bindParam(3, $sessionUserType, PDO::PARAM_STR);
		$update_notified->bindParam(4, $username, PDO::PARAM_STR);
		$update_notified->bindParam(5, $listing_date, PDO::PARAM_STR);
		$update_notified->bindParam(6, $filing_no, PDO::PARAM_STR);
