		...
		<div class="col-sm-8">
			<select name="bench_code" class="bisht form-control" id="test"   onChange="javascript:submitForm();">
				...
				if($_SESSION['menuaccess_codeall'] == '11' || $_SESSION['menuaccess_codeall'] == '6'){
					$query = "select * from $schemas.bench_nature where display=? and bench_code = 3 order by bench_code ASC";
				}else{
					$query = "select * from $schemas.bench_nature where display=? and bench_code != 3 order by bench_code ASC";
				}
				$sqlm=$db->prepare($query);
				$display='Y';
...
<?php
	if($_SESSION['menuaccess_codeall'] == 11 || $_SESSION['menuaccess_codeall'] == 6)
		$query = "select * from $schemas.court where mapped_with = $user_court order by court_no";
	else
