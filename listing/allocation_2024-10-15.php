...
function get_court($db,$schemas,$user_court){
	$courts = $db->prepare("select * from $schemas.court where court_no = $user_court order by court_no");
	$courts->execute();
    ...
    <!-- Content Header (Page header) -->
				...
				?>
					...
					<?php
						...
						$courts = get_court($db,$schemas,$user_court);
						if($msghash !='')
						{
						...
						<!--<div class="search_bar">
							<button type="button" class="btn btn-default" onClick="return search_bar();">search</button>
						</div>
						<form class="form-inline" id="search_form" style='display:none;'>
						  ...
						  <button type="button" class="btn btn-primary" onClick="return search_by_case_no();">search</button><span>
						  <button type="button" class="btn btn-danger" onClick="return dismiss();">Cancel</button></span>
						</form>-->
 ...
 <script src="../assets/js/jquery.simplePagination.js"></script>
	<script>
		...
		function search_by_case_no(){
			...
			search_data('case_no_search',1,'','',search_case_type,search_case_year,search_case_number); 
			
		}
		
		function search_data(type,item_per_page,listing_date,court_no,search_case_type,search_case_year,search_case_no){
		$.ajax({
		type: "POST",
		url: "get_all_cases_count.php",
		data: {type:type,listing_date:listing_date,court_no:court_no,search_case_type:search_case_type,search_case_year:search_case_year,search_case_no:search_case_no},
		success: function (total_items) { 
		   $('.pagination').pagination({
			...
			onPageClick : function(pageNumber) {
				...
				jQuery("#search_data_here").load("get_cases_for_allocation.php?page=" + pageNumber+"&limit="+ item_per_page+"&type="+type+"&listing_date="+listing_date+"&court_no="+court_no+"&search_case_type="+search_case_type+"&search_case_year="+search_case_year+"&search_case_no="+search_case_no);
			},
			onInit :function() {
				jQuery("#search_data_here").html('loading...');
				jQuery("#search_data_here").load("get_cases_for_allocation.php?page=1&limit="+ item_per_page+"&type="+type+"&listing_date="+listing_date+"&court_no="+court_no+"&search_case_type="+search_case_type+"&search_case_year="+search_case_year+"&search_case_no="+search_case_no);
			}
		});
		...
		function reset_all(){
			...
			court_no = $("#court_no").val();
			search_data('reset_cases',100,listing_date,court_no);
			/* $.ajax({
            type: "POST",
            url: "get_cases_for_allocation.php",
