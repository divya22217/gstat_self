...
if($_SESSION['user'] !='' && $_SESSION['location'] !='')
{
	...
	if($type == 'by_court_and_listing_date' || $type == 'reset_cases'){
		...
		$query = "select a.filing_no,a.case_no,a.case_type,a.case_year,a.pet_name,a.res_name,ecd.dt_of_filing,a.location_code,a.regis_date,a.main_case_ia_no,a.list_with_defect,
					...
					(a.location_code is NOT NULL) and  (a.legal_aid IS NULL OR a.legal_aid = 'NULL') and a.status = 'P' $additional_query $user_court_query $napa_query order by CASE WHEN a.case_no ~ '^\d+$' THEN 1  ELSE 2 END, CASE WHEN a.case_no ~ '^\d+$' THEN CAST(a.case_no AS INTEGER) ELSE NULL END ,a.case_type,a.regis_date desc  limit $limit offset $start_from";
		//echo $query;
	...
	if($type == 'all_cases'){
		...
		$query = "select a.filing_no,a.case_no,a.case_type,a.case_year,a.pet_name,a.res_name,ecd.dt_of_filing,a.location_code,a.regis_date,a.main_case_ia_no, a.list_with_defect,
					...
					inner join e_case_detail as ecd on ecd.filing_no = a.filing_no
					where (a.case_no is NOT NULL OR a.case_no != '') and (a.case_year is NOT NULL OR a.case_year != '') and (a.case_type is NOT NULL  and a.case_type != 60) and 
					(a.location_code is NOT NULL) and  (a.legal_aid IS NULL OR a.legal_aid = 'NULL') and a.status = 'P' $user_court_query $napa_query order by CASE WHEN a.case_no ~ '^\d+$' THEN 1  ELSE 2 END, CASE WHEN a.case_no ~ '^\d+$' THEN CAST(a.case_no AS INTEGER) ELSE NULL END,a.case_type,a.regis_date desc limit $limit offset $start_from";
		//echo $query;
		$sql1=$db->prepare($query);
		$sql1->execute();
		$res = $sql1->fetchAll();
	...
	if($type == 'case_no_search'){
		...
		$query = "select a.filing_no,a.case_no,a.case_type,a.case_year,a.pet_name,a.res_name,a.dt_of_filing,a.location_code,a.regis_date,a.main_case_ia_no, a.list_with_defect,
					...
					inner join e_case_detail as ecd on ecd.filing_no = a.filing_no
					where (a.case_no is NOT NULL OR a.case_no != '') and (a.case_year is NOT NULL OR a.case_year != '') and (a.case_type is NOT NULL  and a.case_type != 60) and 
					(a.location_code is NOT NULL) and  (a.legal_aid IS NULL OR a.legal_aid = 'NULL') and a.status = 'P'  and a.case_type = ? and a.case_year = ? and a.case_no = ? and a.location_code = ? $user_court_query $napa_query order by a.regis_date desc limit $limit offset $start_from";
		$sql1=$db->prepare($query);
	...
	if($type == 'all_cases' || $type == 'by_court_and_listing_date' || $type == 'reset_cases' || $type == 'case_no_search') {
		...
		// check if BO exists
			$query = "select count(*) from gst_ecase_assign_tobo_office where filingno = ?"; 
			$sql1=$db->prepare($query);
			$sql1->bindParam(1, $filing_no, PDO::PARAM_INT);
			$sql1->execute();
			$is_bo = $sql1->fetchColumn();
		// check BO exists end
		...
		<tr>
			...
			<td>
				<?php if(($case_type == '1' && $is_bo) || $case_type != '1') { 
					$bo_text = '';
				?>
				<input class="checkbox cases" data-list_with_defect = "<?php echo $lwd; ?>" name="checkbox[]" type="checkbox" id="checkbox_<?php echo $filing_no; ?>" value="<?php echo $filing_no; ?>" >
			<?php } else{
				$bo_text = "<em style='color:red;'> BO not assigned</em>";
			} ?>
			</td>
			<td><?php echo $filing_no.$bo_text; ?></td>
			<?php if($lwd == 'LWD') { ?>
			...
			<td>
				<?php
				if($filing_date!='')
				{ echo date('d/m/Y h:i A',strtotime($filing_date)); }?>
			</td>
			<td>
				<?php
				if($regis_date!='')
				{ 
					echo date('d/m/Y h:i A',strtotime($regis_date));
					 }?>
			</td>
			<td>
				<?php
