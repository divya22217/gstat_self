				...
				<div class="box box-success">
					<div class="box-body">
						...
						<section class="content-header">
							<center><b>Transfer case from one court to another of same branch for same listing date</b></center>
						</section>
						<?php
						...
						<form action="transfer_case_action.php" method='POST' id='transfer_case'>
							...
							<div class="row" style="padding:25px;display:none;" id="transfer_div">
								<label>Transfer To : </label> <br />
								<div class="row" id="transfer_to"></div>
								<center>
									<button type="Submit" name='transfer' id='transfer' class='btn btn-sm btn-success'>Transfer</button>
								</center>
							</div>
						</form>
		...
		<script>
			...
			function submitForm() {
				...
				if (bench_no) {
					$.ajax({
						type: "POST",
						url: "../listing/get_cases_by_bench.php",
						data: {
							listing_date: listing_date,
							bench_no: bench_no
						},
			...
			$(document).on("change", "#select_all", function(e) {
				var $inputs = $('#cases_table_body input[type=checkbox]');
				...
				if ($('.cases:checked').length > 0) {
					$("#transfer_div").css('display', 'inherit');
					getbench('listing_date', 'transfer_to');
				} else {
					$("#transfer_div").css('display', 'none');
					$("#transfer_to").html('');
				}
			...
			$(document).on('change', '#cases_table_body input[type=checkbox]', function() {
			...
			$(document).ready(function(e) {
				...
				$(document).on('submit', '#transfer_case', function(e) {
					e.preventDefault();
					var formdata = new FormData(this);
					swal({
							title: "Are You Sure",
							text: "Do you want to Transfer The Cases",
							type: "info",
