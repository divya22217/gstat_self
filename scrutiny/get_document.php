...
$user_court = $_SESSION['user_court'];
	$query = "select court_no from $schemas.court where mapped_with = ?";
	$court_query = $db->prepare($query);
	$court_query->bindParam(1, $user_court, PDO::PARAM_INT);
	$court_query->execute();
	$mapped_user_court = $court_query->fetchColumn();
	...
	//var_dump($validatekey);
		...
		if(empty($_GET["rowcount"])) {
			if($_SESSION['menuaccess_codeall'] =='2' || $_SESSION['menuaccess_codeall'] =='20'){
				  ...
				  $st_count->bindParam(8, $mapped_user_court, PDO::PARAM_STR);
			...
			if($_SESSION['menuaccess_codeall'] =='11'){
				$display='1';
				 ...
				 $doc_level='1';
				  ...
				  $st_count->bindParam(9, $mapped_user_court, PDO::PARAM_STR);
				  $st_count->bindParam(10, $fictitious, PDO::PARAM_STR);
				  $st_count->bindParam(11, $location_access, PDO::PARAM_STR);
				  $st_count->execute();
			}
			$st_count->execute();
		...
		$perpageresult = $perPage->getAllPageLinks($_GET["rowcount"], $paginationlink,$pagination_setting='');   
		 ...
		 // echo "select distinct(du.miscellenous_no) as miscellaneous_no,du.document_filed_date from document_upload as du left join e_case_detail as ecd on ecd.filing_no = du.filing_no left join e_not_found as enf on enf.fictitious_filing_no = du.filing_no where du.filing_no !=? and du.display=? and du.scrutiny=? and (du.doc_level IS NULL or du.doc_level = '') and du.miscellaneous_ref_no is NOT NULL and du.doc_flag = ? and du.miscellenous_no != ? and ((ecd.location_id = ? and (ecd.court = $user_court or ecd.court = $mapped_user_court)) or (substring(du.filing_no from 1 for 2)= ? AND enf.location_id = ?)) $filing_no_query order by du.document_filed_date desc limit $perPage->perpage offset $start";    
          $st51=$db->prepare("select distinct(du.miscellenous_no) as miscellaneous_no,du.document_filed_date from document_upload as du left join e_case_detail as ecd on ecd.filing_no = du.filing_no left join e_not_found as enf on enf.fictitious_filing_no = du.filing_no where du.filing_no !=? and du.display=? and du.scrutiny=? and (du.doc_level IS NULL or du.doc_level = '') and du.miscellaneous_ref_no is NOT NULL and du.doc_flag = ? and du.miscellenous_no != ? and ((ecd.location_id = ? and (ecd.court = ? or ecd.court = ?)) or (substring(du.filing_no from 1 for 2)= ? AND enf.location_id = ?)) $filing_no_query order by du.document_filed_date desc limit $perPage->perpage offset $start");
		  ...
		  $st51->bindParam(8, $mapped_user_court, PDO::PARAM_STR);
		 ...
		 $doc_level='1';
		  ...
		  $st51->bindParam(9, $mapped_user_court, PDO::PARAM_STR);
		  $st51->bindParam(10, $fictitious, PDO::PARAM_STR);
		  $st51->bindParam(11, $location_access, PDO::PARAM_STR);
          $st51->execute();
