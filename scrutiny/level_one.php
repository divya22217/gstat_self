...
myFunction();
  ...
  */
	...
	$st1 = $db->prepare("select a.filing_no,a.filingnumberia,a.dt_of_filing,a.case_type_nclat as case_type,
		a.case_no,a.act_id,a.patially_defect_remark,a.patially_defective,b.act_name,a.e_reference_no,a.boofficefound,a.remark_return_cases,a.defect_docs,a.supply_disputed_questions from e_case_detail as a
	...
	while ($row = $st1->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT)) {
		...
		$supply_disputed_questions = $row['supply_disputed_questions'];
		$remark_return_cases = $row['remark_return_cases'];
		if($boofficefound == '1')
			$juridection_text = 'Wrong jurisdiction selection';

		if($supply_disputed_questions == '1'){
			$supply_disputed_questions_text = 'This is a place of supply issue. Please scrutinize/check place of supply jurisdiction first, once verified then proceed for rest defect checklist next';
		}
	...
	if(!empty($supply_disputed_questions_text)){ ?>
		<div style="
		background: #fff6ce;
		margin: 30px 40px;
		padding: 10px 20px;
		...
		font-weight: bold;
		color: #e92f18;
		"> <?php echo  $supply_disputed_questions_text; ?></h4></div>
