...
// echo "select * from $schemas.objection_details where filing_no= '$filing_no' and entry_dt=$notification_datez and level_level=$ll and
 ...
 $stv = $db->prepare("select * from $schemas.document_objection_details where miscellaneous_no= ? and date(entry_dt)=? and level_level=? and
 		objection_sub_code=? order by edt asc limit 15");
...
$check_list_count++;
 ...
 //$sql ="select * from $schemas.objection_details where filing_no='$filing_no' and entry_dt='$notification_datez' and level_level='$ll' and
  //		objection_sub_code='$lls' and  status <>'' order by cast(objection_code as integer) ASC ";
			
  $stv = $db->prepare("select * from $schemas.document_objection_details where miscellaneous_no= ? and entry_dt=? and level_level=? and
  		objection_sub_code=? and  status <>''  order by cast(objection_code as integer) ASC");
  $stv->bindParam(1, $miscellaneous_no, PDO::PARAM_STR);
  $stv->bindParam(2, $notification_datez, PDO::PARAM_STR);
  $stv->bindParam(3, $ll, PDO::PARAM_STR);
  $stv->bindParam(4, $lls, PDO::PARAM_STR);
