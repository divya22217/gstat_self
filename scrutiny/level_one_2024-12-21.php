					...
					if ($ccase == '4') {
						...
						while ($rowa = $sth->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT)) {
							...
							<?php if($case_type == '1' && ($id_check == '4' || $id_check == '15')) { ?>
								...
								?>
								<textarea class="comment" id="comment" name="comment[]" onChange="makeUppercase(this)" cols="30" rows="1" style="background-color: silver;"><?php echo htmlspecialchars($comment); ?></textarea>
								</td>
							...
							<font face="Verdana, Arial, Helvetica, sans-serif" size="4" color="red">Defects in
							</td>
							<td>
							<select style="display:none;width: 502px;
							height: 152px;" class="rr" name="cause_no[]" id="cause_no" style="width: 620px; height: 200px" onFocus="SetBg(this)" onChange="get_document(this.value,'<?php echo $filing_no; ?>');" onBlur="UnSetBg(this)" multiple>
							<?php
							...
							while ($row = $st1->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT)) {
								...
								if (in_array($cause_no, $explode_cause_no)) {	$sc_marked_defects .= $e_name.' , ';		print "selected";}?>><?php echo strtoupper($e_name); ?></option>		<?php
							}
							?>
							...
							<font color="red" size="4">For Multiple Defects Select (ctrl+left mouse click)</font></b>
							</td>
							</div>
							</tr>
							<tr>
