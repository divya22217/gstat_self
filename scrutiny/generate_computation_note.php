...
<div class="content-wrapper">
 ...
 $c_case = isset($_REQUEST['c_case']) ? $_REQUEST['c_case'] : '1';
    ...
    ?>
                ...
                <?php 
if(isset($c_case) &&  $c_case == '1' &&  $c_case != '' &&  $_SESSION['menuaccess_codeall'] =='2') { 
?>
                <table id="example1" class="table" >
                    ...
                    <tbody>
                        <?php
					$case_data = defect_free_cases_for_note($db, $schemas, $type='main');
							...
							$subject_id = $row['subjectia'];
								?>
										<tr style="background-color: #BDFCC9;">
											...
											<td>
												<div class="sparkbar" data-color="#00a65a" data-height="20">
													<?php echo $subject_id."  ". get_subject($db,$subject_id); ?></div>
											</td>
											<td>
											<?php 
											...
											if((empty($get_computation_note) || $get_computation_note['in_registrar'] == '0')){
											?>
												<h3><span class="label label-info">
												<a style="color: #FFFFFF;" href="javascript:void(0);" onClick="return comp_note('<?php echo $filing_no; ?>','<?php echo $case_type; ?>','../ajax/computation_note.php');" ><?php echo $btn_text; ?></a>
												</span></h3>
											<?php } 
										...
										<?php	}	
											/* if(!empty($remarks)){ ?>
												<h3><span class="label label-info">
												<a style="color: #FFFFFF;" href="javascript:void(0);" onClick="return show_remarks('<?php echo $filing_no; ?>','<?php echo $case_type; ?>','ajax/computation_note.php');" >Show Remarks</a>
												</span></h3>
										<?php	}	 */
										?>
											</td>
									...
									if (in_array($case_type, $main_cases))
									{
									 ...
									 if(!empty($connected_ia)){
										foreach($connected_ia as $key=>$row){
											...
											$pet_name = get_party($db,$filing_no_ia,$party_flag='P',$party_serial_no=1);
											$res_name = get_party($db,$filing_no_ia,$party_flag='R',$party_serial_no=1);
											$case_title = $pet_name." <b>VS</b> ".$res_name;
