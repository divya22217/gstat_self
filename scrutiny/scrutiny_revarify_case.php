 ...
 <div class="form-group">
       ...
       function defect_submit()
       {
       	...
       	{	
       	 ...
       	 action = "scrutiny_revarify_case_action.php?test="+status1;
       	submit();
...
for (i = 0; i < acc.length; i++) {
  ...
  <form name="form2" method="post" action="scrutiny_revarify_case_action.php" >
...
$stqq = $dbonline->prepare("select count(filing_no) from e_case_detail  where filing_no=?");
  ...
  $scrutiny='0';
  	...
  	$sthrm=$dbonline->prepare("select * from document_upload  where uniqueid=? and scrutiny=? and display=? ");
  	...
  	{
  		 $sub_doc_type=$rowa['doctype'];
                 $documentuploadmodelid=$rowa['documentuploadmodelid'];
                 $original_file11=$rowa['original_file'];
  ...
  ?>
                  <input type="hidden" name="documentuploadmodelid[]" value="<?php echo $documentuploadmodelid; ?>" />
  ...
  <a href="https://efiling.nclt.gov.in/dms-ecourt/ecourt-search-within-dms?applno=<?php echo htmlspecialchars(htmlentities($fileupload_uniqueid));?>&status=P&j_key=vVl%2FAz1yGsjOAG18WDeScg%3D%3D&j_securityKey=6D6C069D681B40DBF95CAD7B3ED71BE1A46F0A7036BC711860B00BAAE50FE8A4%21TZFIMZbTiUtqXMfARJ1DGgyNicWFwYkwTA0ip%2FQ8Wns%3D" target="_blank">
      ...
      <?php echo $original_file11; ?>
      &nbsp;(&nbsp;<?php echo htmlspecialchars(strtoupper($e_document_name_print));?>
  &nbsp;)&nbsp;</a> 
                  <br>
<?php 
        }
...
<?php
 ...
 $comment[$j] = isset($_REQUEST['comment'][$j]) ? $_REQUEST['comment'][$j] : $comments; ?> 
   <textarea name="comment[<?php echo $j;?>]" onChange="makeUppercase(this)" cols="30" rows="1" style="background-color: silver;">
<?php if($comment[$j]!="") echo htmlspecialchars($comment[$j]);?></textarea>
  </td>
...
<b><font color="red">Re- Scrutiny Verify.</font></b>
</td>
