...
function get_filing_no($db,$schema,$case_no,$case_year,$case_type,$status){
   ...
   while ($row51 = $st51->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
		  ...
		  $get_filing_type=$db->prepare("select filing_no,documentuploadmodelid,doctype,uniqueid,document_filed_date from document_upload  where miscellenous_no =? and doc_flag = ? and document_filed_date = ?");
