...
$filing_no_backpage_print = htmlspecialchars(
base64_decode($filing_no_backpage)
);

echo "<center></br><font color='red' size='4'>" .
...
<?php echo htmlspecialchars($miscellaneous_no); ?>
&nbsp;&nbsp;
<?php
$get_case_info = $db->prepare(
"select case_type,filing_no,main_case_ia_no as ia_ma_filing_no from $schemas.case_detail where filing_no=?"
);
...
$st1 = $db->prepare(
"select * from e_case_detail where filing_no=? and location_id=? "
);
...
$st33 = $db->prepare(
"select * from e_cases_party where filing_no=? and party_flag=? and party_serial_no=? "
);
...
?>
<font color="#0000FF">
<?php echo htmlspecialchars_decode(strtoupper($E_nameP)) .
"&nbsp; Vs. &nbsp;" .
htmlspecialchars_decode(strtoupper($E_nameR)); ?>
</font>
</h4>
...
<form name="form2" method="post" action="document_scrutiny_action.php">
	<input type="hidden" name="form2" value="<?php echo htmlspecialchars(
$form2
); ?>" />
	<input type="hidden" name="filing_no_next" value="<?php echo htmlspecialchars(
$filing_no_next
); ?>" />
	<table width="100%">
...
<?php
$st = $db->prepare(
"select *  from document_upload where filing_no=? and scrutiny=? and display=?  and miscellenous_no = ?"
);
...
$stqq = $db->prepare(
"select e_document_name from e_document_type  where e_document_type=?"
);
...
$casenosql = $db->prepare(
"select case_type, case_no, case_year, location_code from $schemas.case_detail where filing_no='$filing_no_fou'"
);
$casenosql->execute();
$row = $casenosql->fetch();
...
$casetypesql = $db->prepare(
"select short_name from case_type where id = '$casetype'"
);
$casetypesql->execute();
...
$get_mis_no_doc = $db->prepare(
"select *  from document_upload where filing_no=? and miscellaneous_ref_no=? and scrutiny=? and display=?  "
);
$get_mis_no_doc->bindParam(1, $filing_no_fou, PDO::PARAM_STR);
$get_mis_no_doc->bindParam(
2,
$miscellaneous_ref_no_post,
PDO::PARAM_STR
);
...
//echo "select *  from document_upload where filing_no='$filing_no_fou' and scrutiny='$scrutiny' and display='$display'  ";
$get_mis_no_doc = $db->prepare(
"select *  from document_upload where filing_no=? and miscellaneous_ref_no=? and scrutiny=? and display=? and subdoctype=? and party_type IN (select party_flag from e_master_govt_body) "
);
$get_mis_no_doc->bindParam(1, $filing_no_fou, PDO::PARAM_STR);
$get_mis_no_doc->bindParam(
2,
...
<?php
$scr = $db->prepare(
"select level_level from $schemas.scrutiny_doc where filing_no = ? and miscellaneous_no = ?"
);
$scr->bindParam(1, $filing_no, PDO::PARAM_STR);
$scr->bindParam(2, $miscellaneous_no, PDO::PARAM_STR);
$scr->execute();
...
$sql = "select status,comments,comment_registrar,status_registrar from $schemas.document_objection_details where filing_no=? and miscellaneous_no = ? and objection_code=?  and scrutiny_type='D'";
$objections_query = $db->prepare($sql);
$objections_query->bindParam(1, $filing_no, PDO::PARAM_STR);
$objections_query->bindParam(2, $miscellaneous_no, PDO::PARAM_STR);
$objections_query->bindParam(3, $id_check, PDO::PARAM_STR);
$objections_query->execute();
$objections = $objections_query->fetch();
$sc_status = $objections["status"];
$ar_status = $objections["status_registrar"];
$sc_comment = $objections["comments"];
$ar_comment = $objections["comment_registrar"];
?>
				<tr>
					...
					<?php if ($previous_scrutiny_level == "2") { ?>
						...
						<td width="7%">
							...
							<p><?php echo $ar_status; ?></p>
						
						</td>

						<td width="20%">

						<p><?php echo htmlspecialchars($ar_comment); ?></p>
...
cursor: pointer;">
							...
							<option value="YES" <?php if ($sc_status == "YES" || empty($sc_status)) {
								echo "selected";
								} ?>>YES
							</option>
							<option value="NO" <?php if ($sc_status == "NO") {
								echo "selected";
								} ?>>
					...
					<td width="20%">
						...
						<textarea name="comment[]" onChange="makeUppercase(this)" cols="30" rows="1" data-id="<?php echo $id_check; ?>" id="comment_<?php echo $id_check; ?>" style="background-color: silver;"><?php echo $sc_comment; ?></textarea>
					</td>


				</tr>

				<input type="hidden" name="id_check[]" maxlength="3" readonly="readonly" value="<?php echo htmlspecialchars(
htmlentities($id_check . "," . "gen")
); ?>" size="2" />

				<input type="hidden" name="filing_no" value="<?php echo htmlspecialchars(
htmlentities($scrutinized_filing_no)
); ?>" />
...
<tr>
	<td>
		<?php
$st2 = $db->prepare(
"select * from e_case_detail_fees where filing_no=? "
);
...
<tr>
<td>
Notified Date: <input id="in_notification_date" type="text" name="notification_date" readonly="readonly" value="<?php echo htmlspecialchars(
htmlentities($cur_date1)
...
<form action="https://efiling.gstat.gov.in/dmsgstat/dashboard" method="POST" target="_blank" id="frm_dms">
...
<form action="https://efiling.gstat.gov.in/CISCasePreview.drt" method="POST" target="_blank" id="previewCIS">
<input type="hidden" id="filling_no" name="filingNo" value="" />
</form>

<form action="https://efiling.gstat.gov.in/previewReceipt.drt" method="POST" target="_blank" id="previewReceipt">
