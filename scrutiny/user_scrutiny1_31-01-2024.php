 ...
 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <!-- Default box -->
      <div class="col-md-12">
	  <div style='text-align:center;'><a href="../index.php" class="text-danger font-weight-bold"> << BACK << </a></div>
      <div class="box">
...
include '../db_inc2.php';
	...
	$E_party_flag1='P';
	  $E_party_serial_no1='1';
		...
		$st33->bindParam(1, $show_party_filing_no, PDO::PARAM_STR);
		...
		$st34->bindParam(1, $show_party_filing_no, PDO::PARAM_STR);
...
<tbody>
    <tr>
      <td colspan="2">
        <table cellpadding="0" cellspacing="0" border='1' class="tbl-accordion-nested">
         <thead>
            <tr>
              <td  class="tbl-accordion-section">Documents </td><td><b> Case Detail </b></td><td><b> Payment Details </b></td>
            </tr>
			<tr>
			<?php 
  ...
  while ($rowa = $sthr->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
  {
	...
	$form_status="I";
	 ...
	 $st=$dbonline->prepare("select *  from document_upload where filing_no=? and scrutiny=? and display=?  and (miscellaneous_ref_no IS NULL or miscellaneous_ref_no = '')");
...
<font color="#900C3F" size="3">&nbsp;&nbsp;
  &nbsp;&nbsp;View</a>
	...
	<td>
		<a  target="_blank" href="https://efiling.nclat.gov.in/previewCIS.drt?filingNo=<?php echo $filing_no ?>"><font color="#900C3F" size="3">&nbsp;&nbsp;View </a>
	</td>
	<td>
	<a  target="_blank" href="https://efiling.nclat.gov.in/previewReceipt.drt?filingNo=<?php echo $filing_no ?>"><font color="#900C3F" size="3">&nbsp;&nbsp;View</a>
	</td>
 ...
 $level_level = 0;
    ...
    $check_record_exist = $db->prepare("select display from $schemas.draft_objection_details where filing_no=? and objection_code=? and objection_sub_code=? and user_id=? and level_level=? and form_type = ?");
          ...
          $check_record_exist->execute();
					...
					if($display_for_chk){
				 $sql="select status,comment,scrutiny_correction from $schemas.draft_objection_details where filing_no='$filing_no' and objection_code='$id_check' and objection_sub_code='$obj_sub_code'and user_id='$userid' and level_level='$level_level' and  form_type = '$frm_type'";
					}else{

 //$sql="select status,doc_comments,scrutiny_correction from $schemas.objection_details where filing_no='$filing_no' and objection_code='$id_check' and objection_sub_code='$obj_sub_code' and form_type = '$frm_type'";
 $sql="select status,doc_comments,scrutiny_correction from $schemas.objection_details where filing_no='$filing_no' and objection_code='$id_check'";
					}
    ...
    $check_record_exist = $db->prepare("select display from $schemas.draft_objection_details where filing_no=? and objection_code=? and objection_sub_code=? and user_id=? and level_level=? and miscellaneous_ref_no=? and form_type = ?");
          ...
          $check_record_exist->execute();
					$display_for_chk= $check_record_exist->fetchColumn();
					if($display_for_chk){
				 $sql="select status,comment from $schemas.draft_objection_details where filing_no='$filing_no' and objection_code='$id_check' and objection_sub_code='$obj_sub_code'and user_id='$userid' and level_level='$level_level' and miscellaneous_ref_no='$miscellaneous_ref_no_post' and  form_type = '$frm_type'";
					}else{

 $sql="select status,doc_comments from $schemas.objection_details where filing_no='$filing_no' and objection_code='$id_check' and objection_sub_code='$obj_sub_code' and miscellaneous_ref_no='miscellaneous_ref_no_post' and form_type = '$frm_type'";
					}
...
$sql="select status,doc_comments from $schemas.objection_details where filing_no='$filing_no' and objection_code='$id_check' and objection_sub_code='$obj_sub_code' and miscellaneous_ref_no='$miscellaneous_ref_no_post' and form_type='$form_type' ";
					}
...
{
	$st1=$dbonline->prepare("select * from scrutiny_status where is_active = true order by id");
}
...
<b><font color="red">List With Defects</font></b>
</td>
...
<td style="display: block; "  id="befornotification">
	...
	if($_SESSION['menuaccess_codeall'] == '20')
	 $button_text = "Reject";
		...
		$button_text = "UPDATE SCRUTINY";
		?>
	 <td> 	<input id="submit_final" type="submit" name="submit_final"
 class="submit" value="<?php echo $button_text; ?>" style="
