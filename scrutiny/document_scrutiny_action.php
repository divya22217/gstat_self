...
$count_scrutiny = $db->prepare("select count(*) from $schemas.scrutiny_doc where filing_no =? and miscellaneous_no = ?");
...
$case_type,$aa,$yes,$doc_level,$scrutiny_type,$miscellaneous_no));
...
if($count_scrutiny == '0'){
	...
	$st13->execute();
  	...
  	$query = "update $schemas.scrutiny_doc set notification_date = ?, objection_status = ?, defects = ?, level_level = ? , user_id = ? , updated_at = now() where filing_no = ? and miscellaneous_no = ?";
  		$st13=$db->prepare($query);
		$st13->bindParam(1, $notification_date1, PDO::PARAM_STR);
		$st13->bindParam(2, $obj_status, PDO::PARAM_STR);
		$st13->bindParam(3, $obj_status, PDO::PARAM_STR);
		$st13->bindParam(4, $doc_level, PDO::PARAM_STR);
	...
	$doc_flag = '1';
		...
		$st1x111 =$db->prepare("update document_upload set doc_level=?, defect_docs = ? where  miscellenous_no=? and scrutiny=? and display=?  and doc_flag = ? ");
	$st1x111->bindParam(1, $doc_level, PDO::PARAM_STR);
	$st1x111->bindParam(2, $docs_defect, PDO::PARAM_STR);
	$st1x111->bindParam(3, $miscellaneous_no, PDO::PARAM_STR);
	$st1x111->bindParam(4, $scrutiny_dc, PDO::PARAM_STR);
	$st1x111->bindParam(5, $scr_display, PDO::PARAM_STR);
	$st1x111->bindParam(6, $doc_flag, PDO::PARAM_STR);
	$st1x111->execute();
...
{ 
	...
	$iii=0;
	 ...
	 $count_scrutiny = $db->prepare("select count(*) as count from $schemas.scrutiny_doc where filing_no =? and miscellaneous_no = ?");
  $count_scrutiny->bindParam(1, $filing_no, PDO::PARAM_STR);
  $count_scrutiny->bindParam(2, $miscellaneous_no, PDO::PARAM_STR);
  $count_scrutiny->execute();
  $count_scrutiny = $count_scrutiny->fetchColumn();
...
$case_type,$aa,$doc_level,$scrutiny_type,$miscellaneous_no));
...
if($count_scrutiny == '0'){
$st13=$db->prepare("insert into  $schemas.scrutiny_doc (filing_no,notification_date,user_id,objection_status,defects,level_level,miscellaneous_no, obj_tabs)
		values (?,?,?,?,?,?,?,?) ");
$st13->bindParam(1, $filing_no, PDO::PARAM_STR);
$st13->bindParam(2, $notification_date1, PDO::PARAM_STR);
$st13->bindParam(3, $sessionUserType, PDO::PARAM_STR);
...
$st13->bindParam(6, $doc_level, PDO::PARAM_STR);
$st13->bindParam(7, $miscellaneous_no, PDO::PARAM_STR);
$st13->bindParam(8, $obj_tabs, PDO::PARAM_STR);
$st13->execute();
}else{
	$query = "update $schemas.scrutiny_doc set notification_date = ?, objection_status = ?, defects = ?, level_level = ? , user_id = ? , updated_at = now(), obj_tabs = ? where filing_no = ? and miscellaneous_no = ?";
  		$st13=$db->prepare($query);
		$st13->bindParam(1, $notification_date1, PDO::PARAM_STR);
		$st13->bindParam(2, $obj_status, PDO::PARAM_STR);
		$st13->bindParam(3, $obj_status, PDO::PARAM_STR);
		...
		$st13->bindParam(8, $miscellaneous_no, PDO::PARAM_STR);
		$st13->execute();
}
	...
	$doc_flag = 1;
		...
		$st1x111 =$db->prepare("update document_upload set doc_level=?, defect_docs = ? , obj_tabs = ? where  miscellenous_no=?  and scrutiny=? and display=? and doc_flag = ?");
	$st1x111->bindParam(1, $doc_level, PDO::PARAM_STR);
	$st1x111->bindParam(2, $docs_defect, PDO::PARAM_STR);
	$st1x111->bindParam(3, $obj_tabs, PDO::PARAM_STR);
