...
if($location_access == '10'){
$location_code = '99';
}if($location_access == '5'){
$location_code = '98';
}
...
function get_filing_no($db,$schema,$case_no,$case_year,$case_type,$status){
	...
	//var_dump($validatekey);
		...
		$paginationlink = "get_filed_document.php?page=";
		$page = 1;
		if(!empty($_GET["page"])) {
		$page = $_GET["page"];
		}
		...
		if(empty($_GET["rowcount"])) {
				  ...
				  //$doc_level='NULL';
				  $st_count=$dbonline->prepare("select count(distinct(miscellenous_no)) as count from document_upload as du left join e_not_found as enf on enf.fictitious_filing_no = du.filing_no where du.filing_no !=? and du.display=? and du.scrutiny=? and (du.doc_level IS NULL or du.doc_level = '') and du.miscellaneous_ref_no is NOT NULL and du.doc_flag = ? and du.miscellenous_no != ? and (substring(du.filing_no from 1 for 2)=? or (substring(du.filing_no from 1 for 2)=? AND enf.location_id = ?)) $filing_no_query");
				  ...
				  $st_count->bindParam(6, $location_code, PDO::PARAM_STR);
				  $st_count->bindParam(7, $fictitious, PDO::PARAM_STR);
				  $st_count->bindParam(8, $location_access, PDO::PARAM_STR);
				  $st_count->execute();
		...
		$perpageresult = $perPage->getAllPageLinks($_GET["rowcount"], $paginationlink,$pagination_setting='');   
          ...
          $st51=$dbonline->prepare("select distinct(du.miscellenous_no) as miscellaneous_no,du.document_filed_date from document_upload as du left join e_not_found as enf on enf.fictitious_filing_no = du.filing_no where du.filing_no !=? and du.display=? and du.scrutiny=? and (du.doc_level IS NULL or du.doc_level = '') and du.miscellaneous_ref_no is NOT NULL and du.doc_flag = ? and du.miscellenous_no != ? and (substring(du.filing_no from 1 for 2)= ? or (substring(du.filing_no from 1 for 2)= ? AND enf.location_id = ?)) $filing_no_query order by du.document_filed_date desc limit $perPage->perpage offset $start");
		  ...
		  $st51->bindParam(6, $location_code, PDO::PARAM_STR);
		  $st51->bindParam(7, $fictitious, PDO::PARAM_STR);
		  $st51->bindParam(8, $location_access, PDO::PARAM_STR);
   ...
   while ($row51 = $st51->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
		  ...
		  $get_filing_type=$dbonline->prepare("select filing_no,documentuploadmodelid,doctype,uniqueid,document_filed_date from document_upload  where miscellenous_no =? and doc_flag = ? and document_filed_date = ?");
...
<td><?php echo htmlspecialchars($SER_COUT3++);?></td>
		 ...
		 $online_dis = 1;
		 $doc_flag = 1;
       $countdocsql=$dbonline->prepare("select count(*) from document_upload where miscellenous_no= ?  and scrutiny= ? and display= ? and miscellaneous_ref_no is NOT NULL and doc_flag = ?");
...
echo "</span>";		 ?></td>
	  ...
	  <?php
		   ...
		   $casenosql=$db->prepare("select case_type, case_no, case_year, location_code from $schemas.case_detail where filing_no= ? ");
		   $casenosql->bindParam(1, $filing_no2, PDO::PARAM_STR);
				 ...
				 $lcodesql ="select short_name from mater_location_city where city_id = ?";
...
<td><?php echo $case_no_final; ?></td>
	 <td><?php echo htmlspecialchars_decode(strtoupper($E_nameP))."&nbsp;<font color='blue' size='2'><br> Vs. <br> </font>&nbsp;".htmlspecialchars_decode(strtoupper($E_nameR));?></td>        
				 ...
				 ?>
                  ...
                  <td>
<a  onclick="OpenDMSForm('3','<?php echo $filing_no2; ?>','','<?php echo $miscellaneous_no;?>')" style="cursor: pointer">
...
<?php 
  ...
  //$cou++;	
	...
	if($filing_no!='' && strlen($filing_no) < 14){
		  ...
		  $st21=$dbo->prepare("select location_id from e_not_found where fictitious_filing_no =? ");
		...
		<?php 
			$e_not_found_detail=$dbo->prepare("select a.case_no,b.case_type_desc,a.case_year from e_not_found a left join case_type b on b.id = a.case_type where a.fictitious_filing_no= ?");
			 ...
			 <td><?php //echo htmlspecialchars_decode(strtoupper($E_nameP))."&nbsp;<font color='blue' size='2'><br> Vs. <br> </font>&nbsp;".htmlspecialchars_decode(strtoupper($E_nameR));?></td>        
						<!-- <td><?php //echo rtrim($r,','); ?></td>-->
						  ...
						  <td><h3><span class="label label-success">
							...
							<br/>
								<a  href="javascript:void(0);" onClick="return view_misc_docs('<?php echo $filing_no; ?>','<?php echo $miscellaneous_no; ?>','../ajax/computation_note.php','../scrutiny/readpdf_file.php');" >View Docs</a>
							</td>
							<?php 
