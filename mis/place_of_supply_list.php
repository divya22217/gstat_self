...
if($_SESSION['user'] !='' and $_SESSION['location'] !='')
{
	...
	if($validatekey){
				...
				if($selected_option == '1'){
						...
						$query = "select count(*) as count from e_case_detail as ecd   where ecd.is_defective = 1 and ecd.scrutiny_level = 2  and ecd.rejected_from_scrutiny = 0 and ecd.scrutiny_count = 2 and ecd.location_id = ? and ecd.refile_count = 1 and ecd.supply_disputed_questions = 2 and ecd.list_with_defect = 0 ";
						...
						$perpageresult = $perPage->getAllPageLinks($_GET["rowcount"], $paginationlink,$pagination_setting='');
				 		...
				 		$query = "select ecd.filing_no,ecd.allow_refiling_date,date(ecd.dt_of_filing) as dt_of_filing from e_case_detail as ecd   where ecd.is_defective = 1 and ecd.scrutiny_level = 2  and ecd.rejected_from_scrutiny = 0 and ecd.scrutiny_count = 2 and ecd.location_id = ? and ecd.refile_count = 1 and ecd.supply_disputed_questions = 2 and ecd.list_with_defect = 0 order by ecd.dt_of_filing desc limit $perPage->perpage offset $start";
						...
						if(!empty($total_records)){
							foreach($data as $key=>$value){
			                    ...
			                    <a class="label label-danger" style="color: #FFFFFF;" href="javascript:void(0);" onClick="return reject_case('<?php echo $filing_no; ?>','<?php echo $case_type; ?>','../ajax/computation_note.php',2);" >Dismiss as Withdrawn as No Place of Supply dispute is involved</a>
								</td>
								</tr>
								<?php
								$count++;
			...
			if($selected_option == '2'){
						...
						$query = "select count(*) as count from e_case_detail as ecd   where ecd.allow_refiling_date < current_date  and  ecd.is_defective = 1 and ecd.scrutiny_level = 2 and ecd.allow_refiling = 1  and ecd.rejected_from_scrutiny = 0  and ecd.scrutiny_count = 1 and ecd.location_id = ?";
						...
						if(!empty($total_records)){
							foreach($data as $key=>$value){
								...
								$place_of_supply = $value['supply_disputed_questions'];

								$is_place_of_supply = ($place_of_supply == '1')?"YES":"NO";
								...
								<td>
			                    <!--<a class="label label-info" style="color: #FFFFFF;" href="javascript:void(0);" onClick="return register_with_defect('<?php echo $filing_no; ?>','<?php echo $case_type; ?>','../ajax/computation_note.php');" >List case with defect</a>-->
			                    <?php if($place_of_supply == '1'){ ?>
			                    <a class="label label-danger" style="color: #FFFFFF;" href="javascript:void(0);" onClick="return reject_case('<?php echo $filing_no; ?>','<?php echo $case_type; ?>','../ajax/computation_note.php',3);" >Reject as No response from appellant</a>
			                <?php } else { ?>
			                	<a class="label label-danger" style="color: #FFFFFF;" href="javascript:void(0);" onClick="return reject_case('<?php echo $filing_no; ?>','<?php echo $case_type; ?>','../ajax/computation_note.php',4);" >Reject</a>
			             <?php   } ?>
								</td>
								</tr>
								<?php
			...
			if($selected_option == '3'){
						...
						$query = "select count(*) as count from e_case_detail as ecd   where ecd.is_defective = 1 and ecd.scrutiny_level = 2  and ecd.rejected_from_scrutiny = 0 and ecd.scrutiny_count = 2 and ecd.location_id = ? and ecd.refile_count = 1 and ecd.supply_disputed_questions = 1 and ecd.list_with_defect = 0 ";
						...
						$perpageresult = $perPage->getAllPageLinks($_GET["rowcount"], $paginationlink,$pagination_setting='');
				 		...
				 		$query = "select ecd.filing_no,ecd.allow_refiling_date,date(ecd.dt_of_filing) as dt_of_filing from e_case_detail as ecd   where ecd.is_defective = 1 and ecd.scrutiny_level = 2  and ecd.rejected_from_scrutiny = 0 and ecd.scrutiny_count = 2 and ecd.location_id = ? and ecd.refile_count = 1 and ecd.supply_disputed_questions = 1 and ecd.list_with_defect = 0 order by ecd.dt_of_filing desc limit $perPage->perpage offset $start";
						...
						if(!empty($total_records)){
							foreach($data as $key=>$value){
			                    ...
			                    <a class="label label-danger" style="color: #FFFFFF;" href="javascript:void(0);" onClick="return reject_case('<?php echo $filing_no; ?>','<?php echo $case_type; ?>','../ajax/computation_note.php',5);" >Case Deferred</a>
								</td>
								</tr>
								<?php
								$count++;
	...
	<script type="text/javascript">
		function reject_case(filing_no,case_type,url,type){
			let msgtxt = '';
			if(type === 2)
				msgtxt = "If yes then case will be 'Dismissed as Withdrawn as No Place of Supply dispute is involved'";
			else if(type === 3)
				msgtxt = "If yes then case will be 'Rejected as No response from appellant'";
			else if(type === 1)
				msgtxt = "If yes then case will be rejected";
			else if(type === 4)
				msgtxt = "If yes then Case Deferred due to Wrong Applicability of Place of Supply"
		...
		.then((willDelete) => { 
			 if (willDelete) {	
				$.ajax({
					type: "POST",
					url: url,
					data: {action:'reject_case',filing_no:filing_no,case_type:case_type,type:type},
					/* contentType: false,
					cache: false,
					processData:false, */
					dataType: 'json',
