		...
		?>
			<table cellspacing="1" cellpadding="1" border="0" align="center">
				<div id="testdiv" style="visibility: visible;">
					...
					<!--<a href="../mis/causelist_doc.php?list_flag=1&court_num=<?php echo $court_no; ?>&listing_date=<?php echo $listdate_entire; ?>">
						<font size="4" color="red">Export in word</font></a>
					...
					<a href="./causelist1.php" class="back-button">
						<center>BACK</center> 
					</a>
...
order by b.court_no, b.priority asc");
				...
				if ($listbefore == 0 and $court_no == '' and ($listdate_entire != '--' or $listdate_entire != '')) {
					$benchloop1 = $db->prepare("select * from $schemas.bench b ,$schemas.bench_nature bn where 
b.from_list_date=? and b.bench_nature=bn.bench_code b.court_no,
b.priority asc");
								...
								<tr>
									...
									<td width="20%" align="left" valign="top">
										<font face="Verdana" size="2">
											...
											?>
											<!-- <form method='POST' target='_blank' action='http://164.100.59.89/dmsnclat/dashboard'>
<input type='hidden' name='case_filing_no' value='<?php echo $filing_no; ?>'>
<input type='hidden' name='court_no' value='<?php echo $court_no; ?>'>
<input type='hidden' name='item_no' value='<?php echo $serial_no; ?>'>
<input type='hidden' name='step' value='2'>			 
<a  href="javascript:void(0);"  style="cursor: pointer">
<?php echo $CASE_NO ?> dddddd dddd</button></a>
</form> -->

											<?php
											$sthr_aa = $db->prepare("select unique_id_no from e_case_detail  where filing_no=? ");
											$sthr_aa->bindParam(1, $filing_no, PDO::PARAM_STR);
											$sthr_aa->execute();
											$fileupload_uniqueid = $sthr_aa->fetchColumn();
											//$fileupload_uniqueid=$rowa['unique_id_no'];
											?>

											<a onclick="OpenDMSForm('2','<?php echo $filing_no; ?>','<?php echo $pet_name . ' Vs ' . $res_name; ?>','<?php echo $case_numaa; ?>','<?php echo $court_no; ?>','<?php echo $sr_no-1; ?>','<?php echo $_SESSION['jwt_token'] ?>')" style="cursor: pointer">

												<?php echo $CASE_NO . ' ' . $old_case_num . '' . $remarkss . $defective_text; ?>
											</a>
									...
									<td width="40%" align="left" valign="top">
										...
										if (!empty($pet_adv_list)) {
											foreach ($pet_adv_list as $k => $val_adv) { ?>
												<font face="Verdana" size="2">
													<?php
													if (strtolower($val_adv['adv_name']) != strtolower('none')) {
														echo strtoupper($val_adv['adv_name']) . " " . $val_adv['remarks'] . '<br>';
													}
												}
											} else {
	     ...
	     $party_sr_no = htmlspecialchars($row12['pflag']).htmlspecialchars($row12['party_serial_no']);
			 ...
			 $party_sr_no = '';
										...
										if (!empty($res_adv_list)) {
											foreach ($res_adv_list as $k => $val_adv) { ?>
												<font face="Verdana" size="2">
													<?php
													if (strtolower($val_adv['adv_name']) != strtolower('none')) {
														echo strtoupper($val_adv['adv_name']) . "  " . $val_adv['adv_type'] . $val_adv['party_serial'] . " " . $val_adv['remarks'] . '<br>';
													}
												}
											} else {
                                            ...
                                            select a.rep_code,b.party_serial_no,b.party_flag,a.party_flag as pflag,a.remarks from e_more_representative_gst_nodal as a left join e_cases_party as b on b.id = a.party_code where a.filing_no=? and a.party_flag='R' and a.show_in_causelist='t'");
												...
												<?php
													if (strtolower($pet_advname22) != strtolower('none')) {
														echo strtoupper($pet_advname22) . " " . $party_sr_no . " " . $row12['remarks'] . '<br>';
													}
												}
											}
												?>
								...
								<?php
										...
										<td width='20%' align='left' valign='top'>
											<font face='Verdana' size='2'><a href='javascript::void();'><?php echo connected_case_no($schemas, $db, $conected_case_record); ?></a>
												<?php
		...
		?>
					...
					<script>
						function OpenDMSForm(step, filing_no, cause_title, case_no, court_no, item_no, token) {
