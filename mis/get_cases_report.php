		...
		if($sessionUserType == $user_id){
			...
			if($selected_type){
				...
				<?php	if($is_partially_defective == 1){
					...
					if(strtoupper($defect_status) == strtoupper('Defective') || strtoupper($defect_status) == strtoupper('Partially Defective')){
					...
					echo "<td style='color:$defect_status_color;'>$defect_status<br/>";

						if(!empty($defect_path)) {
						?>
							<span style='color:<?php echo $defect_status_color; ?>'><a href="javascript:void(0);" onClick="view_pdf('<?php echo urlencode($defect_path); ?>')">Defect Pdf</a>
							<br/>
