...
function get_e_case_detail($db,$filing_no){
	$query = "select supply_disputed_questions,refile_count from e_case_detail as where filing_no = ?";
    $gst_detail = $db->prepare($query);
    $gst_detail->bindParam(1,$filing_no,PDO::PARAM_STR);
    $gst_detail->execute();
    $gst_data = $gst_detail->fetch();
...
function allow_refiling_date($db,$listing_date){
	$query = "WITH RECURSIVE working_days AS (
    SELECT $listing_date AS check_date,
...
$allow_refiling_date = allow_refiling_date($db,$list_date_link);
if($is_exists_in_case_gen == 0){
	...
	if($list_with_defect == 1 && empty($registration_date) && $action_type == '45' ){
		...
		$query = "update e_case_detail set allow_refiling = 0 = null, allow_refiling_date = null where filing_no = ?";
	 	$std=$db->prepare($query);
		$std->bindParam(1, $filing_no, PDO::PARAM_STR);
		$std->execute();
	}
...
if($disposal_nature != '46'){
	...
	if($condonation_delay_flag  && $disposal_nature == '3'){
		...
		if(($disposal_nature == '38' || $disposal_nature == '51') && $is_main_case == 'M'){

			if($disposal_nature == '51')
				$reject_type = 5;
			else
				$reject_type = 6;
...
foreach ($put_api_cases as $key => $filing_no) {
	$url = "https://uat-efiling.gstat.gov.in/efiling/getdataapl02b.drt?filingNo=$filing_no&schema=$schemas";
		callApiAsync($url);
}
