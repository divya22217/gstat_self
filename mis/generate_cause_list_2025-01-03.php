...
<script type="text/javascript" src="../court/tinymce/js/tinymce/tinymce.min.js"></script>
<?php 
...
<div class="container-fluid">
    <table cellspacing="1" cellpadding="1" border="0" align="center">
        <div id="testdiv" style="visibility: visible;">
            ...
            <a style="float:right;" href="javascript::void('0');" onClick="return final_causelist('<?php echo $court_no; ?>','<?php echo $list_date; ?>');">
                <font size="4" color="red">
                    Finalize Causelist</font>
            </a>
        </span>
...
?>
        ...
        <tr>
            <td align="center" colspan="7">
                <font face="Verdana" color="red" size="4"></font>
            </td>
        </tr>
        <?php
}
...
?>
        <tr>
            <td align="center" colspan="7" style="font-size:20px;"><b><u>
...
?>
        ...
        <tr>
            <td align="center" colspan="7" style="font-size:16px;">
                <b><u><?php echo strtoupper($causelisthead.' DATED '.$causelist_display_date.' ('.$dayOfWeek.')'); ?></u></b>
                </font>
            </td>
        </tr>
        <tr>
            <td align="center" colspan="7" style="font-size:16px;">
                <b><u><?php echo strtoupper($display_court_text.' '.$bench_hearder_remark); ?></u></b>
                </font>
            </td>
        </tr>
        <?php
...
$countzz=1;
 ...
 $sql_allocation="select a.filing_no ,a.remarks,a.list_with_defect  from $schemas.case_allocation_temp a,
...
?>
            ...
            <tr>
                <td width="10%" align="left" valign="top">
                    <font size='2'><?php echo htmlspecialchars($sr_no++).".";?></font>
                </td>
...
$serial_no=$sr_no-1;
$dms_link = '#';
//$dms_link = generate_dms_link($db,$db,$schemas,$filing_no,$main_location_code);
...
?>
                        ...
                        <a href="#" style="cursor: pointer" onclick="OpenDMSForm('2','<?php echo $filing_no; ?>','<?php echo $pet_name.' Vs ' .$res_name ;?>','<?php echo $case_numaa; ?>','<?php echo $court_no; ?>','')">

                            <font color="#900C3F" size="2"><?php echo $CASE_NO.' '.$old_case_num.' '.$remarkss.$defective_text; ?>
                        </a>
...
// if main case has ia 
    ...
    if(!empty($main_case_fn)) {
        $selected_main_child = selected_child_cases($schemas,$db,$main_case_fn,1,$listdate_entire,$bench_code1,$list_flag,$court_no,$filing_no);
         if(!empty($selected_main_child)){
            echo show_selected_child_cases($schemas,$db,$main_case_fn,1,$listdate_entire,$bench_code1,$list_flag,$court_no,$filing_no);
        }
    }
    //echo main_cases_child($schemas,$db,$filing_no,$status = 'P');
...
?>
                ...
                <td width="40%" align="left" valign="top">
                    <?php 
$st121=$db->prepare("select a.rep_code,b.party_serial_no,b.party_flag,a.party_flag as pflag,a.remarks from e_more_representative as a left join e_cases_party as b on b.id = a.party_code where a.filing_no=? and a.party_flag='R' and a.show_in_causelist='t'");
    ...
    while ($row121 = $st121->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
    {
         ...
         if(!empty(htmlspecialchars($row121['party_serial_no']))){
         $party_sr_no = htmlspecialchars($row121['pflag']).htmlspecialchars($row121['party_serial_no']);
         }else{
             $party_sr_no = '';
         }
    ...
    $stqq121->execute();
     ...
     ?>
                    <font face="Verdana" size="2">
                        <?php 
      if(strtolower($res_advname22) != strtolower('none')) { echo strtoupper($res_advname22)."  ".$party_sr_no." ".$row121['remarks'].'<br>'; }
    }    
?>
                    </font>
                </td>
                <td width="40%" align="center" valign="top">
                <button type="button" id="remove_<?php echo $filing_no; ?>" class="btn btn-sm btn-danger" name="remove_<?php echo $filing_no; ?>" onClick="return remove_to_fresh_cases('<?php echo $filing_no; ?>','<?php echo $listdate_entire; ?>','<?php echo $listbefore; ?>','<?php echo $court_no; ?>','<?php echo $list_flag; ?>','<?php echo $bench_code1; ?>')">Remove</button><br/><br/>
                <button type="button" id="set_child_cases_<?php echo $filing_no; ?>" class="btn btn-sm btn-primary" name="set_child_cases__<?php echo $filing_no; ?>" onClick="return show_child_cases('<?php echo $filing_no; ?>','<?php echo $listdate_entire; ?>','<?php echo $listbefore; ?>','<?php echo $court_no; ?>','<?php echo $list_flag; ?>','<?php echo $bench_code1; ?>','')">Select Application to show in Causelist</button><br/><br/>
                <button type="button" id="advocate_<?php echo $filing_no; ?>" class="btn btn-sm btn-primary" name="advocate_<?php echo $filing_no; ?>" onClick="return show_advocates('<?php echo $filing_no; ?>','<?php echo $listdate_entire; ?>','<?php echo $listbefore; ?>','<?php echo $court_no; ?>','<?php echo $list_flag; ?>','<?php echo $bench_code1; ?>')">Select Counsel to show</button><br/><br/>
                <?php if(!empty($main_case_fn)) { ?>
                    <button type="button" id="set_main_child_cases_<?php echo $main_case_fn; ?>" class="btn btn-sm btn-primary" name="set_main_child_cases__<?php echo $main_case_fn; ?>" onClick="return show_child_cases('<?php echo $main_case_fn; ?>','<?php echo $listdate_entire; ?>','<?php echo $listbefore; ?>','<?php echo $court_no; ?>','<?php echo $list_flag; ?>','<?php echo $bench_code1; ?>','<?php echo $filing_no; ?>')">Select Application to show in Causelist Of Main Case</button><br/><br/>
...
if(!empty($all_connected_cases)){
    ...
    foreach($all_connected_cases as $key=>$con_cases){
        ...
        <td width='20%' align='left' valign='top'>
            <font face='Verdana' size ='2'>
                <a href='javascript::void();'><?php echo connected_case_no($schemas,$db,$conected_case_record); ?></a>
                <?php
                $selected_child = selected_child_cases($schemas,$db,$con_cases['conn_filing_no'],1,$listdate_entire,$bench_code1,$list_flag,$court_no,'');
                 if(!empty($selected_child)){
                    echo show_selected_child_cases($schemas,$db,$con_cases['conn_filing_no'],1,$listdate_entire,$bench_code1,$list_flag,$court_no,'');
                }else{
                //echo main_cases_child($schemas,$db,$filing_no,$status = 'P');
                }
                echo recursive_cases($schemas,$db,$case_type,$con_cases['conn_filing_no']);
                ?>
            </font>
        </td>
        <?php 
        echo "<td width='30%' align='left' valign='top'><font face='Verdana' size ='2'>".$conected_case_record['pet_name']."<br/>VS</br>".$conected_case_record['res_name'].$with_text."</font></td>";
        echo "<td width='40%' align='left' valign='top'>".counsel_parties_draft($schemas,$db,$db,$case_type_array,$conected_case_record['case_type'],$con_cases['conn_filing_no'],'P')."</td><td width='40%' align='left' valign='top'>".counsel_parties_draft($schemas,$db,$db,$case_type_array,$conected_case_record['case_type'],$con_cases['conn_filing_no'],'R')."</td>"; ?>
        <td width='40%' align='center' valign='top'><button type="button" id="set_child_cases_<?php echo $con_cases['conn_filing_no']; ?>" class="btn btn-sm btn-primary" name="set_child_cases__<?php echo $con_cases['conn_filing_no']; ?>" onClick="return show_child_cases('<?php echo $con_cases['conn_filing_no']; ?>','<?php echo $listdate_entire; ?>','<?php echo $listbefore; ?>','<?php echo $court_no; ?>','<?php echo $list_flag; ?>','<?php echo $bench_code1; ?>','<?php echo $con_cases['conn_filing_no']; ?>')">Select Application to show in Causelist</button><br/><br/><button type="button" id="advocate_<?php echo $con_cases['conn_filing_no']; ?>" class="btn btn-sm btn-primary" name="advocate_<?php echo $con_cases['conn_filing_no']; ?>" onClick="return show_advocates('<?php echo $con_cases['conn_filing_no']; ?>','<?php echo $listdate_entire; ?>','<?php echo $listbefore; ?>','<?php echo $court_no; ?>','<?php echo $list_flag; ?>','')">Select Counsel to show</button></td></tr>
    <?php }
}
...
?>
    ...
    if(!empty($causelist_remarks)){
        $causelist_remark = $causelist_remarks['remark'];
        $causelist_remark2 = $causelist_remarks['remark_footer'];
        $type = 'update';
    } else {
        $causelist_remark = '';
        $type = 'add';
            ...
            <form method='post' action='add_causelist_remark_action.php' id='add_causelist_remark_form'>
                <table width="100%" border='0' cellpadding="1" cellspacing="3" align="center">
                    <tr>
                        ...
                        <input type='hidden' name='listing_date_remark' id='listing_date_remark'
                            value='<?php echo $listdate_entire; ?>'>
                        <input type='hidden' name='court_no_remark' id='court_no_remark'
                            value='<?php echo $court_no_remark; ?>'>
                        <input type='hidden' name='list_flag_remark' id='list_flag_remark'
                            value='<?php echo $list_flag_remark; ?>'>
                        <td><textarea rows='2' name='cause_list_remark' id='causelist_remark' class='form-control'
                                placeholder='Type Remark'><?php echo $causelist_remark; ?></textarea></td>
                        
                    </tr>
                </table>
            <br/>
            <!--<textarea rows='2' name='cause_list_remark2' id='cause_list_remark2' class='form-control' placeholder='Type Remark'>
            <?php if(empty($causelist_remark2)){ ?>
            <table width="100%" border='0' cellpadding="1" cellspacing="3" align="center">
                <tr>
                    <td><b>Copy to : -</b></td>
                    <td></td>
                ...
                <tr>
                    <td></td>
                    <td><b>1 . Notice Board</b></td>
                    <td><b>By the order of</b></td>
                </tr>
                <tr>
                    <td></td>
                    <td><b>2 . Website:<a href='https://www.gstat.nic.in' target='_blank'>gstat.nic.in</a></b></td>
                    <td><b>Hon'ble Chairperson</b></td>
                </tr>
            </table>
            <?php } else { echo $causelist_remark2; } ?>
            </textarea>-->
            <br/>
            <table><tr><td style='padding-left:50px;'><button type='submit' name='submit'
                                class='btn btn-primary btn-sm' id='save_causelist_remark_btn'>Save Remark</td></tr></table>
         </form>
            <?php
}else{
...
?>
            ...
            function edit_draft_cause_list(filing_no, listing_date, bench_nature, court_no, list_flag, bench_no) {
                $.ajax({
                    ...
                    beforeSend: function() {
                        $("#edit_draft_cause_list_model_body").html(
            ...
            function remove_to_fresh_cases(filing_no, listing_date, bench_nature, court_no, list_flag, bench_no) {
                 if(confirm("Are you sure, do you want to remove case from causelist!")){
                $.ajax({
                    type: "POST",
                    url: "edit_draft_ajax.php",
                    data: {
                        type: 'back_to_fresh_cases',
                        filing_no: filing_no,
                        listing_date: listing_date,
                        bench_nature: bench_nature,
                        court_no: court_no,
                    ...
                    beforeSend: function() {
                        $("#edit_draft_cause_list_model_body").html(
                            "<h2><center>Removing Case....</center></h2>");
                        $("#edit_draft_cause_list_model").modal('show');
                    },
                    success: function(data) {
                        //console.log(data);
            ...
            function show_child_cases(filing_no, listing_date, bench_nature, court_no, list_flag, bench_no,listed_fn) {
                $.ajax({
                    type: "POST",
                    url: "edit_draft_ajax.php",
                    data: {
                        type: 'set_child_cases',
                        filing_no: filing_no,
                        listing_date: listing_date,
                        bench_nature: bench_nature,
                        court_no: court_no,
            ...
            function show_advocates(filing_no, listing_date, bench_nature, court_no, list_flag, bench_no) {
                $.ajax({
                    type: "POST",
                    url: "edit_draft_ajax.php",
                    data: {
            ...
            function final_causelist(court_no,listing_date){
                $.ajax({
                    type: "POST",
                    url: "../listing/generate_cause_list_action.php",
                    data: {
                        court: court_no,
                        next_list_date: listing_date
                    },
                    beforeSend: function() {
                        $("#edit_draft_cause_list_model_body").html(
                        "<h2><center>Finilizing Causelist....</center></h2>");
                        $("#edit_draft_cause_list_model").modal('show');
                    },
                    success: function(data) {
                        $("#edit_draft_cause_list_model_body").html(data);
             ...
             $( document ).ready(function() {
                 tinymce.init({
                                ...
                                menu : {
                                    ...
                                    toolbar1: "insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent |  image fontselect fontsizeselect | anchor",
            ...
            <div class="modal fade" id="edit_draft_cause_list_model" role="dialog">
                <div class="modal-dialog" style="width:90%">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button"
                                class="btn btn-danger close_edit_draft_cause_list_model">Close</button>
                        </div>
                    </div>
            ...
            <form action="https://e-commcourt.gov.in/dmsgstat/dashboard" method="POST" target="_blank" id="frm_dms">
