...
if($_SESSION['user'] !='' and $_SESSION['location'] !='')
{
	...
	if($validatekey){
				...
				if($start < 0) $start = 0;
				$count = $start +1;
				$query = "select count(*) as count from e_case_detail as ecd where ecd.apl_02b_accept_reject = 2";
				...
				$perpageresult = $perPage->getAllPageLinks($_GET["rowcount"], $paginationlink,$pagination_setting='');
		 		...
		 		$query = "select ecd.filing_no,ecd.case_type_nclat,ecd.filingnumberia,ct.case_type_desc_cis,ecd.apl_02b_form_path,arm.reject_type from e_case_detail as ecd
		 			inner join case_type as ct on ct.id = ecd.case_type_nclat
		 			inner join apl02_reject_master as arm on arm.reject_id = ecd.apl_02b_accept_reject
		 			where apl_02b_accept_reject in (2,3,4,5) and ecd.location_id = ?  order by ecd.dt_of_filing desc limit $perPage->perpage offset $start";
		 		
		 		$st = $db->prepare($query);
		 		$st->bindParam(1, $location_id, PDO::PARAM_INT);
				$st->execute();
				$data = $st->fetchAll();
				...
				foreach($data as $key=>$value){
					...
					$case_type_name = $value['case_type_desc_cis'];
