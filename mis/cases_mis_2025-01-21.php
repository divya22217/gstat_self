...
<div class="wrapper" style="background-color:#ffffff;">
 ...
 ?>                 <?php if($_SESSION['location'] == '1' && $_SESSION['menuaccess_codeall'] == '11') { ?>
                    <tr>
...
list($day,$month,$year)=explode('/',$from_date);
 ...
 if($search_type == '1'){
	...
	where a.dt_of_filing between ? and ? and a.location_id =?
 ...
 if($search_type == '2'){ ?>
	...
	where a.regis_date between ? and ? and a.location_code =?
 ...
 if($search_type == '3'){ ?>
	...
	left join case_type as c on c.id = a.case_type
	left join $schemas.bench_location as d on d.city_id = a.location_code
	where (dis.disposal_date between ? and ?) and a.location_code =? and dis.court_no = ?
 ...
 if($search_type == '4') { ?>
	...
	a.case_no,a.case_year,d.bench_location_name,a.status,ecd.act_id,ma.act_name,a.main_case_ia_no,a.transfrred_case_type_short,ct.display_court_text,rank() over (partition by cp.filing_no
				...
				where (cp.listing_date between ? and ? ) and a.filing_no != ? and a.filing_no is not null and a.filing_no != ? and length(a.filing_no) = ? and a.status != ? $ct_qyery
						  ) as dt
...
if($search_type == '5'){ ?>
	...
	left join case_type as c on c.id = a.case_type
	left join $schemas.bench_location as d on d.city_id = a.location_code
	where od.order_type = ? and (od.order_date between ? and ?) and a.location_code =?
 ...
 if($search_type == '6'){ ?>
	...
	left join case_type as c on c.id = a.case_type
	left join $schemas.bench_location as d on d.city_id = a.location_code
	where od.author_by = ? and (od.order_date between ? and ?) and a.location_code =?
	and a.filing_no != ? and a.filing_no is not null and a.filing_no != ? and length(a.filing_no) = ?  and a.status != ? $ct_qyery order by od.order_date";
	//echo $query;
	$report_data = $db->prepare($query);
