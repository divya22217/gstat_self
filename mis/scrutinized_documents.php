    ...
    <section class="content">
      ...
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">View Scrutinized Documents </h3>
        ...
        <div class="box-body">
			...
			<div class="row">
				<div class="col-sm-3">
					...
					<label class="label-control col-sm-6" for="start_date">Per Page</label>
				...
				<div class="col-sm-3">
					<div class="form-group">
						<label class="label-control col-sm-4" for="start_date">From</label>
						<div class="form-group">
							<input type="text" autocomplete="off" class="form-control datepicker" name="start_date" id="start_date" placeholder="Enter Scrutiny From Date">
						</div>
					</div>
				</div>
				<div class="col-sm-3">
					<div class="form-group">
						<label for="to_date" class="label-control col-sm-4">To:</label>
						<div class="form-group">
							<input type="text" autocomplete="off"  name="to_date" class="form-control datepicker" placeholder="Enter Scrutiny To Date" id="to_date">
						</div>
					</div>
				</div>
				<div class="col-sm-3" style="margin-top:2%;">
					<div class="form-group">
						...
						<div class="form-group"><div class="form-group">
							<button type="button" class="btn btn-success" onClick="return search_by_date();">Search</button>
							<button type="button" class="btn btn-success" onClick="return reset();">Reset</button>
						</div>
					</div>
					</div>
				</div>
...
$( document ).ready(function() {
	...
	getdata(defect_status,radioValue,item_per_page,from_date,end_date);
});
...
$('input[type=radio][name=cases]').change(function() {
	...
	var defect_status = $("input[name='defect_status']:checked").val();
	var item_per_page = $("#number_of_records").val();
	getdata(defect_status,this.value,item_per_page,from_date,end_date);
});

$('input[type=radio][name=defect_status]').change(function() {
	var from_date = $("#start_date").val();
	var end_date = $("#to_date").val();
	var radioValue = $("input[name='cases']:checked").val();
	var item_per_page = $("#number_of_records").val();
	getdata(this.value,radioValue,item_per_page,from_date,end_date);
});
...
function reset(){
	...
	$("#to_date").val('');
	var defect_status = $("input[name='defect_status']:checked").val();
	getdata(defect_status,radioValue,20,from_date='',end_date='');
}
...
function getresult(url){
	...
	var end_date = $("#to_date").val();
	var item_per_page = $("#number_of_records").val();
	getdata(defect_status,radioValue,item_per_page,from_date,end_date,url);
}

function getdata(defect_status,radioValue,item_per_page,from_date,end_date,url='get_documents_report.php?page='){
	var user_id = '<?php echo $sessionUserType ?>';
	var token = '<?php echo $token ?>';
	 $.ajax({
            type: "POST",
            url: url,
            data: {rowcount:$("#rowcount").val(),user_id:user_id,form_key:token,radioValue:radioValue,from_date:from_date,end_date:end_date,defect_status:defect_status,item_per_page:item_per_page},
  ...
  function viewpdf(pdfpath,filing_no,misc_no)
  {
	  $("#modal_body").html('');
	   var path = pdfpath+misc_no+".pdf";
	  //alert(path);
