...
<?php 
   $sthr_aa=$dbonline->prepare("select unique_id_no from e_case_detail  where filing_no=? ");
...
<a  onclick="OpenDMSForm('http://efiling.nclt.gov.in/dms-ecourt/ecourt-search-within-review-dms','<?php echo $filing_no; ?>','<?php echo $fileupload_uniqueid; ?>','','<?php echo $case_numaa; ?>','<?php echo $case_type;?>','<?php echo $pet_name ?>','<?php echo $pet_name."  "; ?>Vs.<?php echo "  ".$res_name;?>','P','vVl/Az1yGsjOAG18WDeScg==','!TZFIMZbTiUtqXMfARJ1DGgyNicWFwYkwTA0ip/Q8Wns=')" style="cursor: pointer">
<?php echo $CASE_NO.$defective_text; ?> 
</a>
...
<?php }
	...
	if(!empty($causelist_remark)){
		echo "<h4><b>$causelist_remark</b></h4>";
	}
