...
if($_SESSION['user'] !='' and $_SESSION['location'] !='')
{
	...
	if($validatekey){
				...
				if($start < 0) $start = 0;
				$count = $start +1;
				$query = "select count(*) as count from $schemas.case_detail as cd inner join $schemas.case_no_generation as cn on cn.filing_no = cd.filing_no
				...
				$perpageresult = $perPage->getAllPageLinks($_GET["rowcount"], $paginationlink,$pagination_setting='');
		 		...
		 		$query = "select cd.filing_no,cd.case_no,cd.case_year,cd.case_type,ecd.filingnumberia,cd.regis_date,ecd.apl_02b_form_path from $schemas.case_detail as cd inner join $schemas.case_no_generation as cn on cn.filing_no = cd.filing_no
		 			inner join e_case_detail as ecd on ecd.filing_no = cd.filing_no  order by regis_date desc limit $perPage->perpage offset $start";
