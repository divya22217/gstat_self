...
$pdfpath=$_SERVER['HTTP_HOST'].'/nclat/casedoc/defects/';
...
if($_SESSION['user'] !='' and $_SESSION['location'] !='')
{
	...
	function get_cause_title($dbonline,$filing_no){
		$st=$dbonline->prepare("select case_title,case_type,dt_of_filing from e_case_detail where  filing_no=?");
	...
	$main_case_filing_no = $dbonline->prepare("select filing_no,is_filed_in_main_case from case_detail_ma_ia where filing_no_ia_ma = ?");
	...
	$check_type = $main_case_filing_no['is_filed_in_main_case'];
	if($check_type == 'Y'){
		return $main_case_filing_no['filing_no'];
	}else{
	return get_main_filing_no($dbonline,$main_case_filing_no['filing_no']);
	}
}
	...
	function generate_case_no($schemas,$db,$filing_no){
		$case_no = '';
		$case_detail = $db->prepare("select a.case_no,a.case_year,b.short_name as case_type_name,c.short_name as loc,a.transfrred_case_type_short from $schemas.case_detail as a
	...
	function ia_date_of_filing($dbo,$filing_no){
		$dttt = $dbo->prepare("select payment_date::TIMESTAMP::DATE from case_detail_ma_ia where filing_no_ia_ma = ?");
	...
	if($validatekey){
		...
		//if($sessionUserType == $user_id){
			...
			if($selected_type){
				...
				$paginationlink = "get_documents_report.php?page=";
				$page = 1;
				if(!empty($_GET["page"])) {
				$page = $_GET["page"];
				}
				...
				if(empty($_GET["rowcount"])) {
					if($from_date == '' || $end_date == ''){
						$count_query = "select count(distinct(miscellaneous_no)) as count from $schemas.$table where miscellaneous_no is not null and miscellaneous_no != '' $objection_status_query";
						...
						$count_query = "select count(distinct(miscellaneous_no)) as count from $schemas.$table where miscellaneous_no is not null and miscellaneous_no != '' and  $date_column between ? and ? $objection_status_query";
						$st_count=$db->prepare($count_query);
						//$st_count->bindParam(1, $user_id, PDO::PARAM_STR);
				...
				if($from_date == '' || $end_date == ''){
					$query = "select distinct(miscellaneous_no),filing_no,notification_date,objection_status,compliance_date,varifyed_userid from $schemas.$table where miscellaneous_no is not null and miscellaneous_no != '' $objection_status_query order by $date_column desc limit $perPage->perpage offset $start";
					$st=$db->prepare($query);
					...
					$fromdate = date('Y-m-d',strtotime($from_date));
					$enddate = date('Y-m-d',strtotime($end_date));
					$query = "select distinct(miscellaneous_no),filing_no,notification_date,objection_status,compliance_date,varifyed_userid from $schemas.$table where miscellaneous_no is not null and miscellaneous_no != '' and $date_column between ? and ? $objection_status_query order by $date_column desc limit $perPage->perpage offset $start";
					$st=$db->prepare($query);
					//$st->bindParam(1, $user_id, PDO::PARAM_STR);
				...
				foreach($data as $key=>$value){
					if($selected_type == 1){
						$main_filing_no = $value['filing_no'];
						
						$pdfpath=$_SERVER['HTTP_HOST'].'/nclat/casedoc/defects/document/';
					}else{
						$main_filing_no = $value['filing_no'];
						$pdfpath=$_SERVER['HTTP_HOST'].'/nclat/casedoc/defects/document';
					}
					
					$get_casuse_title = get_cause_title($dbo,$main_filing_no);
					/* if($selected_type == 2){
						$get_date_of_filing = ia_date_of_filing($dbo,$value['filing_no']);
					}else{ */
						$get_date_of_filing = $get_casuse_title['dt_of_filing'];
					//}
					/* if($get_date_of_filing < '2019-09-22'){
							continue;
						} */
					$cause_title = cause_title($dbo,$main_filing_no);
					$varifyed_userid = $value['varifyed_userid'];
					$misc_no = $value['miscellaneous_no'];
					$case_no = generate_case_no($schemas,$db,$value['filing_no']);
					echo "<tr>";
					//echo "<td>$count</td>";
					echo "<td>$value[filing_no]</td>";
					echo "<td>$misc_no</td>";
					echo "<td>$case_no</td>";
					...
					if(strtoupper($defect_status) == strtoupper('Defective')){ 
							...
							$rep_miscellaneous_no = str_replace("/","-",$misc_no);
							$pdf_file_name=$rep_miscellaneous_no;
							$defect_path = '/NCLAT_Documents/CIS_Documents/casedoc/defects/document/'.$rep_miscellaneous_no.'.pdf';
					?>
						<td><a href = "javascript:void(0);" onClick="return view_pdf('<?php echo urlencode($defect_path); ?>')" ><input type='button' class='btn btn-xs btn-danger' value='Defect PDF'></a>
						<br/>
