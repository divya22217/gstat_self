...
function handleCase($db, $filing_no, $dt_of_filing, $tableName, $label1, $label2)
{
	...
	if (!empty($date_data) && is_array($date_data)) {
		$createdDate = (new DateTime($dt_of_filing))->format('Y-m-d');
...
function handleSubOfficeCase($db, $filing_no, $dt_of_filing)
{
	...
	if (!empty($date_data_res) && is_array($date_data_res)) {
		...
		echo calculateDays($db, $formattedDate, "Assignment By Sub Officer");
		echo calculateDays($db, $formattedDate, "Scrutiny");
	} else {
		$formattedDate = (new DateTime($dt_of_filing))->format('Y-m-d');
		echo calculateDays($db, $formattedDate, "Assignment By Nodal Officer");
		echo calculateDays($db, $formattedDate, "Scrutiny");
	}
}
								...
								<td>
									...
									<h4>
										...
										?>
													...
													if ($row['transfer_case_level'] == 4 && $row['is_transfered'] == true) {
														$filing_no_remark = base64_encode($filing_no);
													?>

														<!-- target="_blank" href="./custom/verification_remarks.php?filing_no=<?php //echo $filing_no_remark; 
																																?>" -->
														<span class="label label-default" style="background:green;">
															<a style="color:white !important;font-weight:bold;" href="javascript:void(0)" onclick="fn_verified('<?php echo $filing_no_remark; ?>','<?php echo $pet_name; ?>','<?php echo $res_name; ?>')">
																Verified </a>
														</span>
													<?php } ?>
													<br /><br />
												...
												?>
													...
													<!-- href="./custom/verification_remarks.php?filing_no=<?php //echo $filing_no_remark; 
																											?>" -->
													<span class="label label-default" style="background:green;">
														<a style="color:white !important;font-weight:bold;" href="javascript:void(0)" onclick="fn_verified('<?php echo $filing_no_remark; ?>','<?php echo $pet_name; ?>','<?php echo $res_name; ?>')">
															Verified </a>
				...
				<div class="modal-body">
					<div id="collapseFour1" role="tabpanel"
						...
						<div class="panel-body">
							<div class="row" id="remarks_option">
...
<script>
	...
	function fn_verified(filing_no,pet_name,res_name) {
		$("#edit_group_modal").modal('show');
		$("#remarks_option").html('loading...');
		$("#after_select_data").html('');
		var data = {};
		data['action'] = 'getRemarks';
		data['filing_no'] = filing_no;
		data['pet_name'] = pet_name;
		data['res_name'] = res_name;
		data['schema_name']= '<?php echo $_SESSION['schema_name'];?>';
		$.ajax({
			type: "POST",
			url: "./ajax/ajax.php",
			data: data,
