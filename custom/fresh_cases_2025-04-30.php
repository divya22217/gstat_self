...
// Day calculation function developed by Pavan 08/01/2024
				...
				if (!empty($case_data) && is_array($case_data)) {
					...
					foreach ($case_data as $row) {
						...
						if (!empty($data_assigndate) && is_array($data_assigndate)) {;
							$check_date_asign = getDateTimeline($db, $schemas, 7, $data_assigndate['created_at']);
							...
							if (!empty($data_assigndate) && is_array($data_assigndate)) {
								...
								$check_date_asign = getDateTimeline($db, $schemas, 7, $dt_of_filing);
								$date_assign_formate = $dt_of_filing;
							}
						}
