...
<div class="box-body">
    <div class="table-responsive">
        <table class="table no-margin">
			<tr>
                <td colspan="16" align="left">
                    ...
                    <font face="Verdana" size="2">Case Types:
                        ...
                        <font face="Verdana" size="2">Diary No:
            ...
            <?php 
	$main_cases = main_case_type();
    $total_records = defective_cases($dbonline, $location_access,$from_date,$to_date, 'Y', 'NA', $selected_case_type, '',$filing_no_no);
$limit = 100;
$total_pages = ceil(count($total_records)/$limit);  
if (isset($_GET["page"])) { $page  = $_GET["page"]; } else { $page=1; };  
$start_from = ($page-1) * $limit; 
$sn = $start_from+1;
$case_data = defective_cases($dbonline, $location_access,$from_date,$to_date, 'Y', 'NA', $selected_case_type, $limit,$start_from,$filing_no_no);
if (!empty($case_data) && is_array($case_data)) {
    ...
    foreach ($case_data as $row) {
		...
		$sc_correction =  fn_scrutiny_correction($dbonline, $filing_no);
             ...
             if ($upload_check != '0'  and  ($sc_correction=='' ) ){
                ?>
                            <a style="color: #FFFFFF;"
                                href="./scrutiny/user_scrutiny1.php?ccase=<?php echo htmlspecialchars('1'); ?>&filing_no_next=<?php echo htmlspecialchars($filing_no_send); ?>">Scrutiny</a>
                            <?php
            } else { ?>

                            <a style="color: #FFFFFF;"
                                href="javascript:void(0);" onClick="view_pdf('<?php echo (urlencode($scrutiny_info['defect_pdf_path'])); ?>')">Defect Pdf</a>
