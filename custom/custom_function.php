...
function fresh_cases($dbonline, $location_id, $payment_accept, $filing_no, $app_pet, $limit = null, $from_date, $to_date, $start_from = null, $filing_no_yes = null, $date_range = null, $tab_type = null)
{
    ...
    $zero = 0;
    $is_return = 0;
    $query_q = "SELECT cd.filing_no,cd.dt_of_filing,cd.case_type,cd.case_title,cd.case_type_nclat,cd.filingnumberia,cd.subjectia,cd.casetypeiacontempt,cd.contempt_case_year,cd.contempt_case_no,cd.gst_casetransfer_level,cd.cis_user_id,cd.court,cd.gst_casetransfer_level_verify,cd.is_transfered_verify,
...
{
    $query = "select * from e_case_detail where filing_no = ?";
...
function save_defective_case_detail($db, $schemas, $filing_no, $get_case_detail, $pet_name, $res_name, $case_year, $reg_no, $regis_date, $userid, $location_id, $status = 'P')
{
    ...
    $allow_refiling = $get_case_detail['allow_refiling'];
    $rejected_from_scrutiny = $get_case_detail['rejected_from_scrutiny'];
    $allow_refiling_date = $get_case_detail['allow_refiling_date'];
...
{
    $one = 1;
    $query = "update e_case_detail set list_with_defect = ?, allow_refiling = 0 where filing_no = ?";
    $up = $db->prepare($query);
...
function reject_case($db, $filing_no,$reject_type)
{
     // Taxpayer accept that case is not "Place of supply" and select 'NO' case marked as defective again (2nd time)
    // rejected_from_scrutiny = 2 means “Dismissed as Withdrawn as No Place of Supply dispute is involved"
    $query = "update e_case_detail set rejected_from_scrutiny = ? where filing_no = ?";
    $up = $db->prepare($query);
    $up->bindParam(1, $reject_type, PDO::PARAM_STR);
    $up->bindParam(2, $filing_no, PDO::PARAM_STR);
    $res = $up->execute();
    return $res;
}

function allow_arn_for_filing($db, $filing_no)
{
    $query = "update e_order_details set allow_filing_arn = 1 , arn_count = arn_count+1 where filing_no = ?";
    $up = $db->prepare($query);
