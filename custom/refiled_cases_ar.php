		...
		<table class="table no-margin table-bordered">
			...
			<tr>
				<td colspan="12">
					...
					<?php
					$total_records = refiled_cases_ar($db, $user_court, $location_access, 'Y', 'NA', $selected_case_type, '', $from_date, $to_date);
			...
			<tbody>
				<?php
				$limit = 100;
				$total_recordss = refiled_cases_ar($db, $user_court, $location_access, 'Y', 'NA',  $selected_case_type, '', $from_date, $to_date, 0, $filing_no_no);
				...
				$sn = $start_from + 1;
				$case_data = refiled_cases_ar($db, $user_court, $location_access, 'Y', 'NA', $selected_case_type, $limit, $from_date, $to_date, $start_from, $filing_no_no);
							...
							<td>
								<?php
									foreach ($categories as $key => $value) {
										echo ++$key.".  ".$value['category_of_case_under_dispute']."</br></br>";
									}
								 ?>
							</td>
								...
								<td>
									...
									<span><?php echo $main_case_for_backlog; ?><span>
											<h3>
												...
												<span class="label label-info">
													<a style="color: #FFFFFF;" href="./scrutiny/level_two.php?ccase=<?php echo htmlspecialchars(1); ?>&filing_no_next=<?php echo htmlspecialchars($filing_no_send); ?>">Scrutiny</a>
												</span>
											</h3>
								</td>
							<?php	} else { ?>
						...
						if (in_array($case_type, $main_cases)) {
							...
							if (!empty($connected_ia)) {
								foreach ($connected_ia as $key => $row) {
									$show_party_filing_no = $main_parent_filing_no = $filing_no;
									if (empty($show_party_filing_no))
										$show_party_filing_no = htmlspecialchars($row['filing_no']);
