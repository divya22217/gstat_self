...
<td><?php echo htmlspecialchars_decode(html_entity_decode($username)); ?>  </td>
...
<?php   if($localadmin ==1)
{?>
<td colspan="1" align="center"><a href="delete_menus.php?ida=<?php echo htmlspecialchars($hash);?>">Menu Modify</a></td>
  <?php }?>
 <!--td colspan="1" align="center">
<?php if($accesspoint1!=''){?>
<a href="#" onclick="javascript:popsurety_window('delete_user.php?ida=<?php //echo htmlspecialchars($hash);?>');">Delete User</a>
    <?php }else{ // echo $login_status;
    } ?>
  </td-->
<td colspan="1" align="center">
...
$sts1 = $db->prepare("select * from users where login_status like ? and localadmin =0 and location=? order by username ASC");
$sts1->bindValue(1, "$s%", PDO::PARAM_STR);
$sts1->bindParam(2, $locationq, PDO::PARAM_STR);
$sts1->execute();
while ($row = $sts1->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
{
...
<td ><?php echo htmlspecialchars_decode(html_entity_decode(strtoupper(str_replace("_"," ",$location_name))));?> </td>
<td ><?php echo htmlspecialchars($fname);?></td>
<td ><a href="#" onclick="javascript:popsurety_window('delete_menus.php?ida=<?php echo htmlspecialchars($hash);?>');">Menu Modify</a>
</td>
<!--td colspan="1" align="center">
<?php if($accesspoint1!=''){?>
<a href="#" onclick="javascript:popsurety_window('delete_user.php?ida=<?php //echo htmlspecialchars($hash);?>');">Delete User</a>
