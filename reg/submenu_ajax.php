...
if ($_REQUEST['action'] == 'submenu_list') {
	$query  = "select submenu_id,submenu_name,display from submenu_r order by display desc";
    $sth = $db->prepare($query);
    $sth->execute();
	$submenu_list = $sth->fetchAll();
    ...
    $menu_id = $data->submenu_id;
    $menu_name = $data->submenu_name;
   
   // $short_name=pg_escape_string($data->short_name);
 //  echo $org_id;die;
    $msg = 'Something Error. Please try again.';
    if ($menu_id != "") {
        $query = "UPDATE submenu_r SET submenu_name= ? WHERE submenu_id =?";
        try {
    ...
    $msg = 'Something Error. Please try again.';
    if ($id != "") {
        $query = "update submenu_r set display = ?  WHERE submenu_id =?";
        try {
