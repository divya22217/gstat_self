...
function get_connected_cases($schemas,$db,$filing_no){
	$stQ = $db->prepare("select a.*,cast(b.case_no as integer) as case_no ,cast(b.case_year as integer) as case_year from $schemas.connected_cases as a
						left join $schemas.case_detail as b on b.filing_no = a.conn_filing_no
						where a.filing_no = ? and a.status = 'C' order by cast(b.case_year as integer),cast(b.case_no as integer) asc");
	$stQ->bindParam(1, $filing_no, PDO::PARAM_STR);
	$stQ->execute();
	$connected_cases=$stQ->fetchAll();
...
function selected_final_cases($schemas,$db,$filing_no,$display,$listing_date,$bench_no,$list_flag,$court_no,$child_or_connected,$listed_filing_no){
	...
	$stq = $db->prepare("select fcc.filing_no,cast(cd.case_no as integer) as case_no,cd.case_type,cast(cd.case_year as integer) as case_year,ct.short_name as case_type_short_name,mlc.short_name,fcc.pet_name,fcc.res_name,fcc.child_or_connected from $schemas.final_child_and_connected_cases as fcc 
						LEFT JOIN $schemas.case_detail as cd on cd.filing_no = fcc.filing_no 
						LEFT JOIN case_type as ct on ct.id = cd.case_type 
						LEFT JOIN mater_location_city as mlc ON mlc.city_id = cd.location_code
						where fcc.parent_filing_no = ? and fcc.display = ? and fcc.listing_date = ? and fcc.bench_no = ? and fcc.list_flag = ? and fcc.court_no=? and fcc.child_or_connected = ? and listed_filing_no = ? order by cast(cd.case_year as integer),cast(cd.case_no as integer) asc");
	}else{
	$stq = $db->prepare("select fcc.filing_no,cast(cd.case_no as integer) as case_no,cd.case_type,cast(cd.case_year as integer) as case_year,ct.short_name as case_type_short_name,mlc.short_name,fcc.pet_name,fcc.res_name,fcc.child_or_connected from $schemas.final_child_and_connected_cases as fcc 
						LEFT JOIN $schemas.case_detail as cd on cd.filing_no = fcc.filing_no 
						LEFT JOIN case_type as ct on ct.id = cd.case_type 
						LEFT JOIN mater_location_city as mlc ON mlc.city_id = cd.location_code
						where fcc.parent_filing_no = ? and fcc.display = ? and fcc.listing_date = ? and fcc.bench_no = ? and fcc.list_flag = ? and fcc.court_no=? and fcc.child_or_connected = ? order by cast(cd.case_year as integer),cast(cd.case_no as integer) asc");
	}
	$stq->bindParam(1, $filing_no, PDO::PARAM_STR);
...
function display_filing_no($filing_no_display){
  ...
  function order_upload_status($schemas,$db,$filing_no,$listdate_entire){
	$order_date = $db->prepare("select flag from $schemas.order_daily where filing_no = ? and order_date = ?");
	$order_date->bindParam(1, $filing_no, PDO::PARAM_STR);
	$order_date->bindParam(2, $listdate_entire, PDO::PARAM_STR);
	$order_date->execute();
...
function fn_sms($db_online, $type, $filename, $filing_no, $subject, $message, $mail_message, $path = '')
{
    ...
    if ($filing_no != '') {
		...
		send_flag,pdf_path,file_name,entry_date,sms_flag,templateid) 
		VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
            $zero = 0;
